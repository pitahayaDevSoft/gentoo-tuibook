...
- [1Informazioni sul filesystem](#Informazioni_sul_filesystem)
  ...
  - [1.4Creare il file fstab](#Creare_il_file_fstab)
    ...
    - [1.4.2Sistemi UEFI](#Sistemi_UEFI)
...
Se il sistema di init che si sta usando è systemd, se le UUID delle partizioni rispettano la Discoverable Partition Specification (DPS) come specificato in [Preparare i dischi](/wiki/Handbook:AMD64/Installation/Disks/it "Handbook:AMD64/Installation/Disks/it"), e se il sistema usa UEFI, allora si può saltare la creazione dell'fstab poichè systemd monta automaticamente le partizioni che rispettano la specifica.
...
Nell'esempio di partizionamento per **amd64**, /boot/ è solitamente la partizione /dev/sda1, con xfs consigliato come filesystem. Esso necessita di essere controllato durante l'avvio, dunque dovremmo scrivere:
...
Di seguito un esempio di file /etc/fstab per un sistema che si avvierà tramite firmware UEFI:

FILE **`/etc/fstab`** **Un esempio completo di /etc/fstab per un sistema UEFI**
...
FILE **`/etc/fstab`** **Esempio di fstab con DPS PARTUUID**
...
Per impostare l'hostname di un sistema che al momento _esegue_ systemd, può essere usato il programma di utilità hostnamectl. Durante il processo di installazione comunque, il comando [systemd-firstboot](/wiki/Handbook:AMD64/Installation/System/it#Init_and_boot_configuration_systemd "Handbook:AMD64/Installation/System/it") deve essere invece usato (vedere più avanti nel manuale).
...
Informazioni più dettagliate sul funzionamento della rete, inclusi argomenti avanzati come accoppiamenti (bonding), interconnessioni (bridging), VLAN 802.1Q o reti wireless sono trattati nella sezione sulla [configurazione di rete avanzata](/wiki/Handbook:AMD64/Networking/Introduction/it "Handbook:AMD64/Networking/Introduction/it").
...
[← Configurare il kernel](/wiki/Handbook:AMD64/Installation/Kernel/it "Previous part") [Home](/wiki/Handbook:AMD64/it "Handbook:AMD64/it") [Installare gli strumenti →](/wiki/Handbook:AMD64/Installation/Tools/it "Next part")