...
Wenn die Installation bei einem der ab hier folgenden Schritte unterbrochen werden sollte, _sollte_ es möglich sein, ab dieser hier Stelle weiterzuarbeiten. Es ist nicht nötig, die Partitionen erneut zu erstellen! [Mounten Sie die Root-Partition](/wiki/Handbook:AMD64/Installation/Disks/de#Mounting_the_root_partition "Handbook:AMD64/Installation/Disks/de") und führen Sie die oben beschriebenen Schritte ab [DNS-Info kopieren](/wiki/Handbook:AMD64/Installation/Base/de#Copy_DNS_info "Handbook:AMD64/Installation/Base/de") erneut aus, um wieder in die neue Gentoo Linux Umgebung zu gelangen. Dieses Vorgehen ist ebenfalls sinnvoll, um Bootloader-Probleme zu beheben. Weitere Informationen erhalten Sie im [chroot](/wiki/Chroot/de "Chroot/de") Artikel.
...
Um eine reine 64-Bit Umgebung zu erhalten, ohne 32-Bit Anwendungen oder Bibliotheken, können Sie ein no-multilib Profil wählen:
...
Als Nächstes wählen Sie das _no-multilib_ Profil:
...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:AMD64/Installation/Base/de#Gentoo_ebuild_repository "Handbook:AMD64/Installation/Base/de") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **amd64** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:AMD64/Installation/Base/de#Choosing_the_right_profile "Handbook:AMD64/Installation/Base/de") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:AMD64/Installation/Base/de#Gentoo_ebuild_repository "Handbook:AMD64/Installation/Base/de") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
DATEI **`/etc/portage/binrepos.conf/gentoo.conf`** **CDN-based binary package x86-64-v3 host example**
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:AMD64/Working/Features/de#Portage_features "Handbook:AMD64/Working/Features/de") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:AMD64/Installation/Base/de#Optional:_Adding_a_binary_package_host "Handbook:AMD64/Installation/Base/de") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
...
[← Installation des Stage Archivs](/wiki/Handbook:AMD64/Installation/Stage/de "Previous part") [Anfang](/wiki/Handbook:AMD64/de "Handbook:AMD64/de") [Konfiguration des Kernels →](/wiki/Handbook:AMD64/Installation/Kernel/de "Next part")