...
If the init system being used is systemd, the partition UUIDs conform to the Discoverable Partition Specification as given in [Preparing the disks](/wiki/Handbook:AMD64/Installation/Disks/cs "Handbook:AMD64/Installation/Disks/cs"), and the system uses UEFI, then creating an fstab can be skipped, since systemd auto-mounts partitions that follow the spec.
...
V našem příkladu dělení oddílů pro architekturu amd64, bývá /boot/ obvykle oddílem /dev/sda1 se souborovým systémem ext2. V průběhu zavádění systému musí být zkontrolován, takže bychom měli zapsat:
...
For systems running OpenRC, a more detailed reference for network setup is available in the [advanced network configuration](/wiki/Handbook:AMD64/Networking/Introduction/cs "Handbook:AMD64/Networking/Introduction/cs") section, which is covered near the end of the handbook. Systems with more specific network needs may need to skip ahead, then return here to continue with the rest of the installation.
...
To set the system hostname for a system currently _running_ systemd, the hostnamectl utility may be used. During the installation process however, [systemd-firstboot](/wiki/Handbook:AMD64/Installation/System/cs#Init_and_boot_configuration_systemd "Handbook:AMD64/Installation/System/cs") command must be used instead (see later on in handbook).
...
[← Konfigurace jádra](/wiki/Handbook:AMD64/Installation/Kernel/cs "Previous part") [Home](/wiki/Handbook:AMD64/cs "Handbook:AMD64/cs") [Instalace nástrojů →](/wiki/Handbook:AMD64/Installation/Tools/cs "Next part")