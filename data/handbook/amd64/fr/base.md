...
- [1Chrootage](#Chrootage)
  ...
  - [1.4Préparer le programme d’amorçage ( _bootloader_)](#Pr.C3.A9parer_le_programme_d.E2.80.99amor.C3.A7age_.28bootloader.29)
    - [1.4.1Système UEFI](#Syst.C3.A8me_UEFI)
    - [1.4.2Système DOS/BIOS dépréciés](#Syst.C3.A8me_DOS.2FBIOS_d.C3.A9pr.C3.A9ci.C3.A9s)
...
Si l’installation de Gentoo est interrompue n’importe où après ce point, il _devrait_ être possible de reprendre l’installation depuis cette étape. Il n'y a pas besoin de refaire le partitionnement des disques ! Il suffit simplement de [monter la partition racine](/wiki/Handbook:AMD64/Installation/Disks/fr#Mounting_the_root_partition "Handbook:AMD64/Installation/Disks/fr") et d’exécuter les commandes ci-dessus depuis la [copie des informations DNS](/wiki/Handbook:AMD64/Installation/Base/fr#Copy_DNS_info "Handbook:AMD64/Installation/Base/fr") pour réintégrer l’environnement de travail. Ceci est également utile pour résoudre les problèmes de chargeur d’amorçage. Plus d’informations peuvent être trouvées dans l’article sur [chroot](/wiki/Chroot "Chroot").
...
Pour les systèmes UEFI, /dev/sda1 a été formatée en FAT32 et sera utilisée comme ESP ( _EFI System Partition_). Si ce n’est pas déjà fait, créer un répertoire /dev/sda1 et monter l’ESP dessus :
...
La configuration pour un _binhost_ se trouve dans le répertoire Portage /etc/portage/binrepos.conf/, qui fonctionne comme mentionné dans la section [dépôt _ebuild_ Gentoo](/wiki/Handbook:AMD64/Installation/Base/fr#Gentoo_ebuild_repository "Handbook:AMD64/Installation/Base/fr").

Lors de la définition d’un hôte de paquets binaires, les deux aspects les plus importants à considérer sont :

1. La cible d’architecture et de profil avec `sync-uri` _compte_ et devrait être identique à l’architecture de l’ordinateur ( **amd64**, dans ce cas) et le profil système sélectionné dans la section [Choisir le bon profil](/wiki/Handbook:AMD64/Installation/Base/fr#Choosing_the_right_profile "Handbook:AMD64/Installation/Base/fr").
2. Choisir un miroir rapide et proche géographiquement va en général diminuer le temps de téléchargement. Consultez l’outil mirrorselect mentionné dans la section [Facultatif : Choisir un mioir](/wiki/Handbook:AMD64/Installation/Base/fr#Gentoo_ebuild_repository "Handbook:AMD64/Installation/Base/fr") ou regardez la [liste des miroirs en ligne](https://www.gentoo.org/downloads/mirrors/) où les URL peuvent être retrouvées.
...
Des fonctionnalités supplémentaires de Portage sera détaillées dans le [prochain chapitre](/wiki/Handbook:AMD64/Working/Features/fr#Portage_features "Handbook:AMD64/Working/Features/fr") du manuel.
...
Les lecteurs qui ont ajouté un hôte de paquets binaires [ci-dessus](/wiki/Handbook:AMD64/Installation/Base/fr#Optional:_Adding_a_binary_package_host "Handbook:AMD64/Installation/Base/fr") peuvent ajouter --getbinpkg (ou -g) dans le but de récupérer les paquets depuis un hôte binaire au lieu de les compiler :
...
[← Installer l’archive _stage3_](/wiki/Handbook:AMD64/Installation/Stage/fr "Previous part") [Accueil](/wiki/Handbook:AMD64/fr "Handbook:AMD64/fr") [Configurer le noyau →](/wiki/Handbook:AMD64/Installation/Kernel/fr "Next part")