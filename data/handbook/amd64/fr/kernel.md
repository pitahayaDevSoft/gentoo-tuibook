...
- [5Configuration et compilation du noyau](#Configuration_et_compilation_du_noyau)
  ...
  - [5.3Installer les sources du noyau](#Installer_les_sources_du_noyau)
    ...
    - [5.3.4Compiler et installer](#Compiler_et_installer)
...
Installer certains micrologiciels nécessite souvent d’accepter la licence associée. Si nécessaire, visitez la [section gestion des licences](/wiki/Handbook:AMD64/Working/Portage/fr#Licenses "Handbook:AMD64/Working/Portage/fr") du manuel pour de l’aide à propos des licences.
...
Les _[noyaux distribués](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel")_, comme [sys-kernel/gentoo-kernel](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel) et [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin) attendent par défaut d’être installés à côté de [initramfs](/wiki/Handbook:AMD64/Installation/Kernel#Initramfs.2Ffr "Handbook:AMD64/Installation/Kernel"). Avant de lancer _emerge_ pour installer le noyau, les utilisateurs doivent s’assurer que [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) est bien configuré pour utiliser un générateur _initramfs_ (par exemple [Dracut](/wiki/Dracut "Dracut")) comme décrit dans la section [_installkernel_](/wiki/Handbook:AMD64/Installation/Kernel#Initramfs.2Ffr "Handbook:AMD64/Installation/Kernel").
...
Après avoir installé le noyau distribué avec succès, il est maintenant possible de continuer dans la prochaine section : [Configurer le système](/wiki/Handbook:AMD64/Installation/System/fr "Handbook:AMD64/Installation/System/fr").

### Approche manuelle

### Installer les sources du noyau

Lors de l’installation et la compilation du noyau pour les systèmes basés sur amd64, Gentoo recommande le paquet [sys-kernel/gentoo-sources](https://packages.gentoo.org/packages/sys-kernel/gentoo-sources).
...
For now please follow installing [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin) via [Distribution\_kernels](/wiki/Handbook:AMD64/Installation/Kernel#Distribution_kernels.2Ffr "Handbook:AMD64/Installation/Kernel").
...
Now it's time to decide if modules signing is required in the steps listed in [here](/wiki/Handbook:AMD64/Installation/Kernel#Optional:_Signed_kernel_modules_2.2Ffr "Handbook:AMD64/Installation/Kernel")

If not, proceed to building described [here](/wiki/Handbook:AMD64/Installation/Kernel#Compiling_and_installing.2Ffr "Handbook:AMD64/Installation/Kernel")
...
{{#switch: amd64 \| amd64 \| arm64 \| riscv \| x86 =
...
Avec cette configuration terminée, il est temps de compiler et installer le noyau. Quitter la configuration et lancer le processus de compilation :

`root #` `make && make modules_install`

**Remarque**

Il est possible de paralléliser la compilation en utilisant make -jX avec `X` correspondant au nombre de tâches en parallèle que la construction est autorisée à lancer. C’est similaire à l’instruction de /etc/portage/make.conf avec la variable MAKEOPTS.

Lorsque le noyau a terminé de compiler, copier l’image dudit noyau dans /boot/. Ceci peut se faire avec la commande make install :

`root #` `make install`

Cette commande va copier l’image du noyau dans /boot. Si [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) est installé /sbin/installkernel sera appelé au lieu de déléguer l’installation au noyau. À la place de simplement copier le noyau dans /boot, [Installkernel](/wiki/Installkernel/fr "Installkernel/fr") installe chaque noyau avec sa version dans le nom de fichier. En supplément, _installkernel_ accomplit des tâches variées pour l’installation du noyau comme : générer un [_initramfs_](/index.php?title=%27%27initramfs%27%27&action=edit&redlink=1 "''initramfs'' (page does not exist)"), compiler une [image unifiée du noyau](/index.php?title=Image_unifi%C3%A9e_du_noyau&action=edit&redlink=1 "Image unifiée du noyau (page does not exist)") et mettre à jour la configuration du programme [d’amorçage](/index.php?title=D%E2%80%99amor%C3%A7age&action=edit&redlink=1 "D’amorçage (page does not exist)").

Continuer l’installation avec [Configurer le système](/wiki/Handbook:AMD64/Installation/System/fr "Handbook:AMD64/Installation/System/fr").

[← Installation du système de base](/wiki/Handbook:AMD64/Installation/Base/fr "Previous part") [Accueil](/wiki/Handbook:AMD64/fr "Handbook:AMD64/fr") [Configurer le Système →](/wiki/Handbook:AMD64/Installation/System/fr "Next part")