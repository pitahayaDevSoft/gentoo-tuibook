...
Now that the partitions have been created, it is time to place a filesystem on them. In the next section the various file systems that Linux supports are described. Readers that already know which filesystem to use can continue with [Applying a filesystem to a partition](/wiki/Handbook:X86/Installation/Disks#Applying_a_filesystem_to_a_partition "Handbook:X86/Installation/Disks"). The others should read on to learn about the available filesystems...
...
Later in the instructions, the proc filesystem (a virtual interface with the kernel) as well as other kernel pseudo-filesystems will be mounted. But first [the Gentoo stage file](/wiki/Handbook:X86/Installation/Stage "Handbook:X86/Installation/Stage") must be extracted.
