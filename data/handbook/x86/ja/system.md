...
- [2ネットワーク接続のための情報](#.E3.83.8D.E3.83.83.E3.83.88.E3.83.AF.E3.83.BC.E3.82.AF.E6.8E.A5.E7.B6.9A.E3.81.AE.E3.81.9F.E3.82.81.E3.81.AE.E6.83.85.E5.A0.B1)
  ...
  - [2.4任意自由選択: PCMCIA を機能させる](#.E4.BB.BB.E6.84.8F.E8.87.AA.E7.94.B1.E9.81.B8.E6.8A.9E:_PCMCIA_.E3.82.92.E6.A9.9F.E8.83.BD.E3.81.95.E3.81.9B.E3.82.8B)
...
init システムに systemd を使用しており、パーティション UUID が [Preparing the disks](/wiki/Handbook:X86/Installation/Disks/ja "Handbook:X86/Installation/Disks/ja") で説明されている Discoverable Partition Specification に適合していて、かつ UEFI を使用しているシステムでは、systemd が仕様に従ってパーティションを自動的にマウントするので、fstab の作成を省略できます。
...
**x86** のパーティション構成の例として、/boot は通常は /dev/sda1 パーティションになり、推奨されるファイルシステムである xfs を使います。このパーティションはブート中にチェックされなければならないので、fstab は以下のようになるでしょう:
...
OpenRC を動かしているシステムについては、ネットワーク設定のためのより詳細なリファレンスが、ハンドブックの終わりのほうの [高度なネットワーク設定](/wiki/Handbook:X86/Networking/Introduction/ja "Handbook:X86/Networking/Introduction/ja") のセクションでカバーされています。より詳細なネットワーク要件のあるシステムは一度そちらへ飛んで、それからここに戻ってきて残りのインストール作業を続ける必要があるかもしれません。
...
現在 _実行中の_ systemd についてシステムのホスト名を設定するには、hostnamectl ユーティリティを使うことができます。しかしながらインストールプロセス中は、 [systemd-firstboot](/wiki/Handbook:X86/Installation/System/ja#Init_and_boot_configuration_systemd "Handbook:X86/Installation/System/ja") コマンドを代わりに使う必要があります (後でハンドブックの該当箇所を参照してください)。
...
bonding、ブリッジ、802.1Q VLAN、無線ネットワークに間するより詳細な情報は、 [追加のネットワーク設定](/wiki/Handbook:X86/Networking/Introduction/ja "Handbook:X86/Networking/Introduction/ja") セクションを参照してください。
...
ここではネットワークインターフェイスがeth0であると仮定していますが、これはシステムによって違います。もし、最近のインストールメディアから起動しているのであれば、インストール時と同じインターフェイス名が使われると思ってよいでしょう。より詳しい情報は [ネットワークインターフェースの命名](/wiki/Handbook:X86/Networking/Advanced/ja#Network_interface_naming "Handbook:X86/Networking/Advanced/ja") セクションを参照してください。
...
### 任意自由選択: PCMCIA を機能させる

PCMCIA のサポートが必要な **x86** システムでは、[sys-apps/pcmciautils](https://packages.gentoo.org/packages/sys-apps/pcmciautils) パッケージをインストールする必要があります。この老朽化した 16 ビット技術へのサポートは、v6.4.0 で [まずは 'char' デバイスから](https://git.kernel.org/pub/scm/linux/kernel/git/gregkh/char-misc.git/commit/?h=char-misc-next&id=9b12f050c76f090cc6d0aebe0ef76fed79ec3f15)、 [本流の Linux カーネルから取り除かれ](https://lore.kernel.org/all/Y07d7rMvd5++85BJ@owl.dominikbrodowski.net/) つつあることに注意してください。
...
[← カーネルの設定](/wiki/Handbook:X86/Installation/Kernel/ja "Previous part") [Home](/wiki/Handbook:X86/ja "Handbook:X86/ja") [ツールのインストール →](/wiki/Handbook:X86/Installation/Tools/ja "Next part")