...
Если установка Gentoo будет прервана где-то после этой точки, _можно_ «продолжить» установку с последнего состояния. Не нужно разбивать диск снова! Просто [смонтируйте корневой раздел](/wiki/Handbook:X86/Installation/Disks/ru#Mounting_the_root_partition "Handbook:X86/Installation/Disks/ru") снова и проделайте предыдущие шаги, начиная с [копирования информации о DNS](/wiki/Handbook:X86/Installation/Base/ru#Copy_DNS_info "Handbook:X86/Installation/Base/ru"), для повторного входа в рабочее окружение. Эти шаги подойдут и для решения проблем с загрузчиком. Больше информации можно найти в статье [chroot](/wiki/Chroot/ru "Chroot/ru").
...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:X86/Installation/Base/ru#Gentoo_ebuild_repository "Handbook:X86/Installation/Base/ru") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **x86** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:X86/Installation/Base/ru#Choosing_the_right_profile "Handbook:X86/Installation/Base/ru") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:X86/Installation/Base/ru#Gentoo_ebuild_repository "Handbook:X86/Installation/Base/ru") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:X86/Working/Features/ru#Portage_features "Handbook:X86/Working/Features/ru") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:X86/Installation/Base/ru#Optional:_Adding_a_binary_package_host "Handbook:X86/Installation/Base/ru") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
...
[← Установка Gentoo файлов установки](/wiki/Handbook:X86/Installation/Stage/ru "Предыдущая часть") [К содержанию](/wiki/Handbook:X86/ru "Handbook:X86/ru") [Настройка ядра Linux →](/wiki/Handbook:X86/Installation/Kernel/ru "Следующая часть")