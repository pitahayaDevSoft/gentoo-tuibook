...
For **x86**, we document how to configure [GRUB](/wiki/Handbook:X86/Blocks/Bootloader/es#Default:_GRUB "Handbook:X86/Blocks/Bootloader/es") for DOS/Legacy BIOS based systems, and [GRUB](#Default:_GRUB), [systemd-boot](#Alternative_1:_systemd-boot) or [EFI Stub](#Alternative_2:_efibootmgr) for UEFI systems.
...
Upon successful installation, the output should match the output of the previous command. If the output does not match exactly, then proceed to [Debugging GRUB](/wiki/Handbook:X86/Blocks/Bootloader/es#Debugging_GRUB "Handbook:X86/Blocks/Bootloader/es"), otherwise jump to the [Configure step](/wiki/Handbook:X86/Blocks/Bootloader/es#GRUB_Configure "Handbook:X86/Blocks/Bootloader/es").
...
To install systemd-boot, enable the [boot](https://packages.gentoo.org/useflags/boot) [USE flag/es (page does not exist)](/index.php?title=USE_flag/es&action=edit&redlink=1 "USE flag/es (page does not exist)") USE flag and re-install [sys-apps/systemd](https://packages.gentoo.org/packages/sys-apps/systemd) (for systemd systems) or [sys-apps/systemd-utils](https://packages.gentoo.org/packages/sys-apps/systemd-utils) (for OpenRC systems):
...
System administrators who desire to take a minimalist, although more rigid, approach to booting the system can avoid secondary bootloaders and boot the Linux kernel as an [EFI stub](/wiki/EFI_stub/es "EFI stub/es").
...
Ensure the [EFI stub](/wiki/EFI_stub/es "EFI stub/es") kernel article has been reviewed _before_ continuing. The kernel must have specific options enabled to be directly bootable by the UEFI firmware. It may be necessary to recompile the kernel in order to build-in this support.
...
To boot the kernel directly from the firmware, the kernel image must be present on the [EFI System Partition](/wiki/EFI_System_Partition "EFI System Partition"). This may be accomplished by enabling the [efistub](https://packages.gentoo.org/useflags/efistub) [USE flag/es (page does not exist)](/index.php?title=USE_flag/es&action=edit&redlink=1 "USE flag/es (page does not exist)") USE flag on [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel). Given that EFI Stub booting is not guaranteed to work on every UEFI system, the USE flag is stable masked, testing keywords must be accepted for installkernel to use this feature.
...
Alternatively, when [installkernel](/wiki/Installkernel "Installkernel") is not used, the kernel may be copied manually to the EFI System Partition, calling it bootx64.efi:
...
For other options that are not covered in the Handbook, see the full list of available [bootloaders](/wiki/Bootloader/es "Bootloader/es").
...
Una vez reiniciado en el nuevo entorno Gentoo, es aconsejable finalizar con [Finalizando la instalación de Gentoo](/wiki/Handbook:X86/Installation/Finalizing/es "Handbook:X86/Installation/Finalizing/es").

[← Instalar las herramientas](/wiki/Handbook:X86/Installation/Tools/es "Previous part") [Inicio](/wiki/Handbook:X86/es "Handbook:X86/es") [Finalizar →](/wiki/Handbook:X86/Installation/Finalizing/es "Next part")