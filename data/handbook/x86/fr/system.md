...
- [2Informations réseau](#Informations_r.C3.A9seau)
  ...
  - [2.4Optionnel : Faire fonctionner PCMCIA](#Optionnel_:_Faire_fonctionner_PCMCIA)
...
Si _systemd_ est utilisé comme système d’initialisation, l’UUID de partition conforme à _Discoverable Partition Specification_ est renseigné dans [préparer les disques](/wiki/Handbook:X86/Installation/Disks/fr "Handbook:X86/Installation/Disks/fr") ; et pour les systèmes UEFI, la création dans _fstab_ peut être passé puisque l’auto-montage 'systemd _respecte les spécifications._
...
Regardons comment noter les options pour la partition /boot/. Ceci n’est qu'un exemple et doit être modifié en fonctions des décisions prises plus tôt dans l’installation. Dans notre exemple de partitionnement **x86**, /boot/ est généralement la /dev/sda1 partition, avec xfs comme système de fichier. Cette partition nécessite d’être vérifiée lors du démarrage et nous écrirons donc :
...
Pour les systèmes _OpenRC_, un guide plus détaillé pour le paramétrage du réseau est disponible dans la [configuration avancé du réseau](/wiki/Handbook:X86/Networking/Introduction/fr "Handbook:X86/Networking/Introduction/fr"), qui est situé à la fin du manuel. Les systèmes qui ont des besoins réseaux plus spécifiques devraient passer ce qui suit, et revenir après avoir configuré le réseau.
...
Pour configurer le nom de l’hôte sur un système utilisant _systemd_, l’utilitaire hostnamectl peut être employé. Cependant, pendant la procédure d’installation, [1er démarrage de _systemd_](/wiki/Handbook:X86/Installation/System/fr#Init_and_boot_configuration_systemd "Handbook:X86/Installation/System/fr") doit être utilisé (voir plus loin dans le manuel).
...
Des informations plus détaillées sur la mise en réseau, incluant des sujets avancés tels que _bonding_, _bridging_, _VLAN_ 802.1Q ou les réseaux sans fil se trouvent dans la section [configuration réseau avancée](/wiki/Handbook:X86/Networking/Introduction/fr "Handbook:X86/Networking/Introduction/fr").
...
Il est supposé que l’interface réseau s’appellera eth0. Cela est, cependant, entièrement dépendant du système. Il est recommandé de partir du principe que l’interface portera la même nom que lors du démarrage depuis le support d’installation si ce dernier est suffisamment récent. Plus d’informations sont accessibles dans la section [Nommage de l’interface réseau](/wiki/Handbook:X86/Networking/Advanced#Network_interface_naming.2Ffr "Handbook:X86/Networking/Advanced").
...
Les systèmes **x86** nécessitant le support PCMCIA devrait installer le paquet [sys-apps/pcmciautils](https://packages.gentoo.org/packages/sys-apps/pcmciautils). Notez que ce support date de la technologie 16 bits et a été [retiré de la branche principale du noyau](https://lore.kernel.org/all/Y07d7rMvd5++85BJ@owl.dominikbrodowski.net/) [à partir de la version v6.4.0](https://git.kernel.org/pub/scm/linux/kernel/git/gregkh/char-misc.git/commit/?h=char-misc-next&id=9b12f050c76f090cc6d0aebe0ef76fed79ec3f15).
...
[← Configurer le noyau](/wiki/Handbook:X86/Installation/Kernel/fr "Previous part") [Accueil](/wiki/Handbook:X86/fr "Handbook:X86/fr") [Installing tools →](/wiki/Handbook:X86/Installation/Tools/fr "Next part")