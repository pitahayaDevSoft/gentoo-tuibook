...
젠투 설치가 여기 어디에선가 멈췄다면 이 단계에서 설치를 '재개'할 수 있"어야"합니다. 디스크 영역을 다시 분할할 필요가 없습니다! 간단하게 [루트 분할 영역 마운트](/wiki/Handbook:X86/Installation/Disks/ko#Mounting_the_root_partition "Handbook:X86/Installation/Disks/ko") 를 진행하고, 작업 환경에 다시 들어오기 전 [#DNS 정보 복사](#DNS_.EC.A0.95.EB.B3.B4_.EB.B3.B5.EC.82.AC) 로 위 단계 진행을 시작하십시오. 이 방법은 부트로더 문제를 해결할 때도 쓸만합니다. 더 많은 내용은 [chroot](/wiki/Chroot "Chroot") 게시글에서 찾아보실 수 있습니다.
...
x86 아키텍처에서 존재하는 프로파일을 확인한 후 사용자는 시스템의 다른 프로파일을 선택할 수 있습니다:
...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:X86/Installation/Base/ko#Gentoo_ebuild_repository "Handbook:X86/Installation/Base/ko") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **x86** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:X86/Installation/Base/ko#Choosing_the_right_profile "Handbook:X86/Installation/Base/ko") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:X86/Installation/Base/ko#Gentoo_ebuild_repository "Handbook:X86/Installation/Base/ko") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:X86/Working/Features/ko#Portage_features "Handbook:X86/Working/Features/ko") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:X86/Installation/Base/ko#Optional:_Adding_a_binary_package_host "Handbook:X86/Installation/Base/ko") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
...
[← 젠투 설치 파일 설치](/wiki/Handbook:X86/Installation/Stage/ko "이전 내용") [처음](/wiki/Handbook:X86/ko "Handbook:X86/ko") [리눅스 커널 설정 →](/wiki/Handbook:X86/Installation/Kernel/ko "다음 내용")