...
- [2네트워크 정보](#.EB.84.A4.ED.8A.B8.EC.9B.8C.ED.81.AC_.EC.A0.95.EB.B3.B4)
  ...
  - [2.6선택: PCMCIA 동작하게 만들기](#.EC.84.A0.ED.83.9D:_PCMCIA_.EB.8F.99.EC.9E.91.ED.95.98.EA.B2.8C_.EB.A7.8C.EB.93.A4.EA.B8.B0)
...
If the init system being used is systemd, the partition UUIDs conform to the Discoverable Partition Specification as given in [Preparing the disks](/wiki/Handbook:X86/Installation/Disks/ko "Handbook:X86/Installation/Disks/ko"), and the system uses UEFI, then creating an fstab can be skipped, since systemd auto-mounts partitions that follow the spec.
...
x86 분할 영역 예제에서, /boot/는 보통 ext2 파일 시스템을 쓰는 /dev/sda1 분할 영역입니다. 부팅 과정에 검사해야 하기 때문에 다음의 대용으로 적어내려가겠습니다:
...
For systems running OpenRC, a more detailed reference for network setup is available in the [advanced network configuration](/wiki/Handbook:X86/Networking/Introduction/ko "Handbook:X86/Networking/Introduction/ko") section, which is covered near the end of the handbook. Systems with more specific network needs may need to skip ahead, then return here to continue with the rest of the installation.
...
To set the system hostname for a system currently _running_ systemd, the hostnamectl utility may be used. During the installation process however, [systemd-firstboot](/wiki/Handbook:X86/Installation/System/ko#Init_and_boot_configuration_systemd "Handbook:X86/Installation/System/ko") command must be used instead (see later on in handbook).
...
PCMCIA 사용자는 [sys-apps/pcmciautils](https://packages.gentoo.org/packages/sys-apps/pcmciautils) 꾸러미를 설치해야합니다.
...
[← 리눅스 커널 설정](/wiki/Handbook:X86/Installation/Kernel/ko "이전 내용") [처음](/wiki/Handbook:X86/ko "Handbook:X86/ko") [시스템 도구 설치 →](/wiki/Handbook:X86/Installation/Tools/ko "다음 내용")