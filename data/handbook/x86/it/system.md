...
- [2Informazioni di rete](#Informazioni_di_rete)
  ...
  - [2.4Facoltativo: Far funzionare PCMCIA](#Facoltativo:_Far_funzionare_PCMCIA)
...
Se il sistema di init che si sta usando è systemd, se le UUID delle partizioni rispettano la Discoverable Partition Specification (DPS) come specificato in [Preparare i dischi](/wiki/Handbook:X86/Installation/Disks/it "Handbook:X86/Installation/Disks/it"), e se il sistema usa UEFI, allora si può saltare la creazione dell'fstab poichè systemd monta automaticamente le partizioni che rispettano la specifica.
...
Nell'esempio di partizionamento per **x86**, /boot/ è solitamente la partizione /dev/sda1, con xfs consigliato come filesystem. Esso necessita di essere controllato durante l'avvio, dunque dovremmo scrivere:
...
[Handbook:X86/Blocks/Fstab/it](/index.php?title=Handbook:X86/Blocks/Fstab/it&action=edit&redlink=1 "Handbook:X86/Blocks/Fstab/it (page does not exist)")
...
Per impostare l'hostname di un sistema che al momento _esegue_ systemd, può essere usato il programma di utilità hostnamectl. Durante il processo di installazione comunque, il comando [systemd-firstboot](/wiki/Handbook:X86/Installation/System/it#Init_and_boot_configuration_systemd "Handbook:X86/Installation/System/it") deve essere invece usato (vedere più avanti nel manuale).
...
Informazioni più dettagliate sul funzionamento della rete, inclusi argomenti avanzati come accoppiamenti (bonding), interconnessioni (bridging), VLAN 802.1Q o reti wireless sono trattati nella sezione sulla [configurazione di rete avanzata](/wiki/Handbook:X86/Networking/Introduction/it "Handbook:X86/Networking/Introduction/it").
...
### Facoltativo: Far funzionare PCMCIA

I sistemi **x86** che necessitano del supporto PCMCIA dovrebbero installare ora il pacchetto [sys-apps/pcmciautils](https://packages.gentoo.org/packages/sys-apps/pcmciautils). Si noti che il supporto per questa tecnologia datata a 16-bit è stata [rimossa dalla linea principale del kernel Linux](https://lore.kernel.org/all/Y07d7rMvd5++85BJ@owl.dominikbrodowski.net/) [a partire dai dispositivi 'char'](https://git.kernel.org/pub/scm/linux/kernel/git/gregkh/char-misc.git/commit/?h=char-misc-next&id=9b12f050c76f090cc6d0aebe0ef76fed79ec3f15) nella versione v6.4.0.
...
[← Configurare il kernel](/wiki/Handbook:X86/Installation/Kernel/it "Previous part") [Home](/wiki/Handbook:X86/it "Handbook:X86/it") [Installare gli strumenti →](/wiki/Handbook:X86/Installation/Tools/it "Next part")