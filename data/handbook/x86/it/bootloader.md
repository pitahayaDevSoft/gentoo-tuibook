...
For **x86**, we document how to configure [GRUB](/wiki/Handbook:X86/Blocks/Bootloader/it#Default:_GRUB "Handbook:X86/Blocks/Bootloader/it") for DOS/Legacy BIOS based systems, and [GRUB](#Default:_GRUB), [systemd-boot](#Alternative_1:_systemd-boot) or [EFI Stub](#Alternative_2:_efibootmgr) for UEFI systems.
...
Upon successful installation, the output should match the output of the previous command. If the output does not match exactly, then proceed to [Debugging GRUB](/wiki/Handbook:X86/Blocks/Bootloader/it#Debugging_GRUB "Handbook:X86/Blocks/Bootloader/it"), otherwise jump to the [Configure step](/wiki/Handbook:X86/Blocks/Bootloader/it#GRUB_Configure "Handbook:X86/Blocks/Bootloader/it").
...
To boot the kernel directly from the firmware, the kernel image must be present on the [EFI System Partition](/wiki/EFI_System_Partition "EFI System Partition"). This may be accomplished by enabling the [efistub](https://packages.gentoo.org/useflags/efistub) [USE flag/it (page does not exist)](/index.php?title=USE_flag/it&action=edit&redlink=1 "USE flag/it (page does not exist)") USE flag on [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel). Given that EFI Stub booting is not guaranteed to work on every UEFI system, the USE flag is stable masked, testing keywords must be accepted for installkernel to use this feature.
...
Una volta effettuato il riavvio nell'ambiente Gentoo appena installato, è ragionevole concludere con [Ultimare l'installazione di Gentoo](/wiki/Handbook:X86/Installation/Finalizing/it "Handbook:X86/Installation/Finalizing/it").

[← Installare gli strumenti](/wiki/Handbook:X86/Installation/Tools/it "Previous part") [Home](/wiki/Handbook:X86/it "Handbook:X86/it") [Concludere →](/wiki/Handbook:X86/Installation/Finalizing/it "Next part")