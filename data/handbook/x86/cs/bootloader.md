...
For **x86**, we document how to configure [GRUB](/wiki/Handbook:X86/Blocks/Bootloader/cs#Default:_GRUB "Handbook:X86/Blocks/Bootloader/cs") for DOS/Legacy BIOS based systems, and [GRUB](#Default:_GRUB), [systemd-boot](#Alternative_1:_systemd-boot) or [EFI Stub](#Alternative_2:_efibootmgr) for UEFI systems.
...
To boot the kernel directly from the firmware, the kernel image must be present on the [EFI System Partition](/wiki/EFI_System_Partition "EFI System Partition"). This may be accomplished by enabling the [efistub](https://packages.gentoo.org/useflags/efistub) [USE flag/cs (page does not exist)](/index.php?title=USE_flag/cs&action=edit&redlink=1 "USE flag/cs (page does not exist)") USE flag on [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel). Given that EFI Stub booting is not guaranteed to work on every UEFI system, the USE flag is stable masked, testing keywords must be accepted for installkernel to use this feature.
...
Jakmile jste zrestartovali do čerstvě nainstalovaného prostředí Gentoo, dokončete vše podle [Dokončení instalace Gentoo](/wiki/Handbook:X86/Installation/Finalizing "Handbook:X86/Installation/Finalizing").

[← Instalace nástrojů](/wiki/Handbook:X86/Installation/Tools/cs "Previous part") [Home](/wiki/Handbook:X86/cs "Handbook:X86/cs") [Dokončení →](/wiki/Handbook:X86/Installation/Finalizing/cs "Next part")