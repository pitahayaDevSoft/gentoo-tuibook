...
- [6부팅](#.EB.B6.80.ED.8C.85)
  - [6.1설치 CD 부팅](#.EC.84.A4.EC.B9.98_CD_.EB.B6.80.ED.8C.85)
...
시작하기 전에, 우선 hppa 장치에 젠투를 성공적으로 설치할 때 필요한 하드웨어 요구 사항을 하나하나 살펴보겠습니다.
...
소형 설치 CD는 install-hppa-minimal-<release>.iso 파일 이름을 지니고 있습니다.
...
스테이지 3 타르볼은 [공식 젠투 미러](https://www.gentoo.org/downloads/mirrors/) en중 어디서든 releases/hppa/autobuilds/ 위치에서 다운로드 할 수 있습니다. 스테이지 파일은 자주 업데이트하며, 설치 CD에는 없습니다.
...
2. ( **hppa/** 와 같은) 대상 아키텍처 관련 디렉터리를 선택하십시오
...
코드 **releases/hppa/autobuilds/current-iso/ 의 다운로드 파일 목록 예제**
...
위 예제에서 install-hppa-minimal-20141204.iso 파일이 소형 설치 CD 그 자체입니다. 그러나 보시다시피, 다른 관련 파일도 있습니다:
...
설치 CD 또는 다른 매체의 부팅에 문제가 있을 경우 [PA-RISC 리눅스 부팅 설명서](http://www.tldp.org/HOWTO/PA-RISC-Linux-Boot-HOWTO/index.html) 를 읽어보십시오.

HPPA 시스템을 부팅하십시오. 부팅 과정에 다음과 같은 메시지가 나타납니다:
...
이 메시지가 나타나면, 옵션 메뉴가 나타날때까지 `Esc` 키를 누른채로 기다리십시오. 시간이 좀 걸릴지 모르니, 조금만 참으십시오. 기본적으로 BOOT\_ADMIN 콘솔에 들어가야 합니다. 옵션 메뉴가 나타나면 _Enter Boot Administration mode_ 를 선택하여 BOOT\_ADMIN 콘솔로 진입하십시오. 결과적으로 '>' 프롬프트가 떠야합니다.

CD-ROM에 젠투 설치 CD를 넣으십시오. CD-ROM 드라이브의 SCSI ID를 모른다면, `search` 명령을 실행했을경우 PA-RISC 스테이션에서 CD-ROM 드라이브의 SCSI ID를 찾습니다.
...
이제 PA-RISC 스테이션에서 부팅할 수 있는 미디어를 표시합니다. 이 명령의 결과 예제 화면은 다음과 같습니다:
...
CD-ROM에서 부팅하려면 관련 장치 경로가 필요합니다. 예를 들어, 위 예제에서 TOSHIBA CD-ROM에서 부팅한다면 다음 명령을 입력하십시오:
...
ipl 키워드(초기 프로그램 로더)는 PALO(PA-RISC boot LOader)가 대화식 모드로 진입하도록 합니다. 이 과정을 통해 커널 부팅 매개 변수와 같은 사항을 바꿀 수 있습니다.

부팅이 성공적으로 끝나면, PALO는 대화식 모드로 시작합니다:
...
이 매개변수는 대부분의 상황에서도 적합합니다.

추가 기능이 필요하다면, 명령줄 뒤에 적절한 키워드를 추가하십시오. 키워드를 추가하려면 마지막 필드를 편집하여 공백을 추가한 후 키워드를 입력하십시오. 현재 구현한 키워드는 설치 CD를 RAM에 불러오는 cdcahe와 불러오지 않을 모듈을 분명히 알려줄 noload=module1\[,module2\[,...\]\] 입니다.
...
이제 커널 부팅 매개변수를 설정했으니, 부팅하십시오.

`(or 'b' to boot with this command line)?` `b`

결과적으로 현재 콘솔에 루트("#") 프롬프트가 떠야합니다. `Alt` + `F2`, `Alt` + `F3`, `Alt` + `F4` 를 눌러 다른 콘솔로 전환할 수 있습니다. `Alt` + `F1` 을 눌러 처음 콘솔로 돌아가십시오.
...
`user $` `links https://wiki.gentoo.org/wiki/Handbook:HPPA/ko`
...
sshd를 사용할 수 있게 하려면, 네트워크가 올바르게 동작해야 합니다. [네트워크 설정](/wiki/Handbook:HPPA/Installation/Networking/ko "Handbook:HPPA/Installation/Networking/ko") 장으로 계속 진행하십시오.

[← 젠투 리눅스 설치 정보](/wiki/Handbook:HPPA/Installation/About/ko "이전 내용") [처음](/wiki/Handbook:HPPA/ko "Handbook:HPPA/ko") [네트워크 설정 →](/wiki/Handbook:HPPA/Installation/Networking/ko "다음 내용")