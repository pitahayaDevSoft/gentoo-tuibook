...
Если в качестве системы инициализации используется systemd, UUID разделов соответствуют Discoverable Partition Specification (как это представлено в разделе [Подготовка дисков](/wiki/Handbook:HPPA/Installation/Disks/ru "Handbook:HPPA/Installation/Disks/ru")) и система использует UEFI, то создание fstab можно пропустить, так как systemd автоматически монтирует все разделы, соответствующие данной спецификации.
...
В **hppa** примере, /boot является обычным /dev/sda2 разделом с рекомендуемой файловой системой xfs. Необходимо проверять его во время загрузки, поэтому мы запишем следующее:
...
Для систем с OpenRC, более подробное руководство по настройке сети доступно в разделе [Расширенная настройка сети](/wiki/Handbook:HPPA/Networking/Introduction/ru "Handbook:HPPA/Networking/Introduction/ru"), который находится ближе к концу руководства. Системы с более специфическими сетевыми потребностями могут пропустить этот раздел, а затем вернуться сюда, чтобы продолжить установку.
...
Чтобы установить имя хоста для системы с _уже запущенным_ systemd, можно воспользоваться утилитой hostnamectl. Однако, во время установки вместо неё следует использовать команду [systemd-firstboot](/wiki/Handbook:HPPA/Installation/System/ru#Init_and_boot_configuration_systemd "Handbook:HPPA/Installation/System/ru") (см. ниже).
...
Больше информации о настройке сети, в том числе об объединении интерфейсов, создании мостов, настройке 802.1Q VLAN и беспроводной сети, рассматриваются в разделе [Настройка сети](/wiki/Handbook:HPPA/Networking/Introduction/ru "Handbook:HPPA/Networking/Introduction/ru").
...
[← Настройка ядра Linux](/wiki/Handbook:HPPA/Installation/Kernel/ru "Предыдущая часть") [К содержанию](/wiki/Handbook:HPPA/ru "Handbook:HPPA/ru") [Установка системных средств →](/wiki/Handbook:HPPA/Installation/Tools/ru "Следующая часть")