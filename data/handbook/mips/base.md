...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:MIPS/Installation/Base#Gentoo_ebuild_repository "Handbook:MIPS/Installation/Base") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **mips** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:MIPS/Installation/Base#Choosing_the_right_profile "Handbook:MIPS/Installation/Base") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:MIPS/Installation/Base#Gentoo_ebuild_repository "Handbook:MIPS/Installation/Base") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:MIPS/Working/Features#Portage_features "Handbook:MIPS/Working/Features") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:MIPS/Installation/Base#Optional:_Adding_a_binary_package_host "Handbook:MIPS/Installation/Base") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
