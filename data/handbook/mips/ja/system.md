...
init システムに systemd を使用しており、パーティション UUID が [Preparing the disks](/wiki/Handbook:MIPS/Installation/Disks/ja "Handbook:MIPS/Installation/Disks/ja") で説明されている Discoverable Partition Specification に適合していて、かつ UEFI を使用しているシステムでは、systemd が仕様に従ってパーティションを自動的にマウントするので、fstab の作成を省略できます。
...
**mips** のパーティション構成の例として、/boot は通常は /dev/sda1 パーティションになり、推奨されるファイルシステムである xfs を使います。このパーティションはブート中にチェックされなければならないので、fstab は以下のようになるでしょう:
...
OpenRC を動かしているシステムについては、ネットワーク設定のためのより詳細なリファレンスが、ハンドブックの終わりのほうの [高度なネットワーク設定](/wiki/Handbook:MIPS/Networking/Introduction/ja "Handbook:MIPS/Networking/Introduction/ja") のセクションでカバーされています。より詳細なネットワーク要件のあるシステムは一度そちらへ飛んで、それからここに戻ってきて残りのインストール作業を続ける必要があるかもしれません。
...
現在 _実行中の_ systemd についてシステムのホスト名を設定するには、hostnamectl ユーティリティを使うことができます。しかしながらインストールプロセス中は、 [systemd-firstboot](/wiki/Handbook:MIPS/Installation/System/ja#Init_and_boot_configuration_systemd "Handbook:MIPS/Installation/System/ja") コマンドを代わりに使う必要があります (後でハンドブックの該当箇所を参照してください)。
...
bonding、ブリッジ、802.1Q VLAN、無線ネットワークに間するより詳細な情報は、 [追加のネットワーク設定](/wiki/Handbook:MIPS/Networking/Introduction/ja "Handbook:MIPS/Networking/Introduction/ja") セクションを参照してください。
...
ここではネットワークインターフェイスがeth0であると仮定していますが、これはシステムによって違います。もし、最近のインストールメディアから起動しているのであれば、インストール時と同じインターフェイス名が使われると思ってよいでしょう。より詳しい情報は [ネットワークインターフェースの命名](/wiki/Handbook:MIPS/Networking/Advanced/ja#Network_interface_naming "Handbook:MIPS/Networking/Advanced/ja") セクションを参照してください。
...
[← カーネルの設定](/wiki/Handbook:MIPS/Installation/Kernel/ja "Previous part") [Home](/wiki/Handbook:MIPS/ja "Handbook:MIPS/ja") [ツールのインストール →](/wiki/Handbook:MIPS/Installation/Tools/ja "Next part")