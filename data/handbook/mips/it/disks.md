...
[Handbook:MIPS/Blocks/Disks/it](/index.php?title=Handbook:MIPS/Blocks/Disks/it&action=edit&redlink=1 "Handbook:MIPS/Blocks/Disks/it (page does not exist)")
...
Una volta che sono state create le partizioni, è ora di applicarci un filesystem. Nella sezione successiva vengono descritti i file system compatibili con Linux. I lettori che sanno già quale filesystem usare possono continuare con [applicare un filesystem ad una partizione](/wiki/Handbook:MIPS/Installation/Disks/it#Applicare_un_filesystem_ad_una_partizione "Handbook:MIPS/Installation/Disks/it"). Gli altri dovrebbero continuare a leggere per comprendere meglio i filesystem disponibili...

### Filesystem

Linux è compatibile con parecchie dozzine di filesystem, sebbene per molti di questi abbia senso impiegarli solo per scopi specifici. Solo alcuni filesystem potrebbero risultare stabili sull'architettura mips - si consiglia di documentarsi sui filesystem e lo stato della loro compatibilità prima di selezionarne uno più sperimentale per partizioni importanti. **XFS è il filesystem consigliato per tutti gli scopi, su tutte le piattaforme.** Quello sotto è un elenco non esaustivo:
...
Per esempio, per avere la partizione root (/dev/sda5) con xfs come nello schema delle partizioni d'esempio, si dovrebbero usare i seguenti comandi:
...
Le installazioni che sono state iniziate in precedenza, ma non hanno terminato tutto il processo possono riprendere da questo punto del manuale. Si usi questo link come permalink [Iniziare da qui per installazioni da riprendere](/wiki/Handbook:MIPS/Installation/Disks/it#Resumed_installations_start_here "Handbook:MIPS/Installation/Disks/it").
...
Più avanti nel manuale, il filesystem proc (un'interfaccia virtuale fornita dal kernel) ed altri pseudo-filesystem del kernel verranno montati. Però prima deve essere estratto il [file stage di Gentoo](/wiki/Handbook:MIPS/Installation/Stage/it "Handbook:MIPS/Installation/Stage/it").

[← Configurare la rete](/wiki/Handbook:MIPS/Installation/Networking/it "Previous part") [Home](/wiki/Handbook:MIPS "Handbook:MIPS") [Installare il file stage →](/wiki/Handbook:MIPS/Installation/Stage/it "Next part")