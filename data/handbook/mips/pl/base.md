...
- [தமிழ்](/wiki/Handbook:MIPS/Installation/Base/ta "கையேடு:MIPS/நிறுவல்/அடிப்படை (100% translated)")
...
If the Gentoo installation is interrupted anywhere after this point, it _should_ be possible to 'resume' the installation at this step. There is no need to re-partition the disks again! Simply check the date and time, [mount the root partition](/wiki/Handbook:MIPS/Installation/Disks/pl#Mounting_the_root_partition "Handbook:MIPS/Installation/Disks/pl") and run the steps above starting with [copying the DNS info](/wiki/Handbook:MIPS/Installation/Base/pl#Copy_DNS_info "Handbook:MIPS/Installation/Base/pl") to re-enter the working environment. This is also useful for fixing bootloader issues. More information can be found in the [chroot](/wiki/Chroot "Chroot") article.
...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:MIPS/Installation/Base/pl#Gentoo_ebuild_repository "Handbook:MIPS/Installation/Base/pl") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **mips** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:MIPS/Installation/Base/pl#Choosing_the_right_profile "Handbook:MIPS/Installation/Base/pl") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:MIPS/Installation/Base/pl#Gentoo_ebuild_repository "Handbook:MIPS/Installation/Base/pl") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:MIPS/Working/Features/pl#Portage_features "Handbook:MIPS/Working/Features/pl") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:MIPS/Installation/Base/pl#Optional:_Adding_a_binary_package_host "Handbook:MIPS/Installation/Base/pl") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
...
[← Installing the stage file](/wiki/Handbook:MIPS/Installation/Stage "Previous part") [Home](/wiki/Handbook:MIPS "Handbook:MIPS") [Configuring the kernel →](/wiki/Handbook:MIPS/Installation/Kernel "Next part")