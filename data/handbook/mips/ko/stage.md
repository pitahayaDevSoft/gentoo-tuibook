...
시스텝 기본 타르볼을 선택하면, 특히 [시스템 프로파일 선택](/wiki/Handbook:MIPS/Installation/Base/ko#.EC.A0.81.EC.A0.88.ED.95.9C_.ED.94.84.EB.A1.9C.ED.8C.8C.EC.9D.BC_.EC.84.A0.ED.83.9D "Handbook:MIPS/Installation/Base/ko") 을 진행할 때, 설치 과정의 나머지 시간을 절약할 수 있습니다. 스테이지 타르볼 선택은 앞으로의 시스템 설정에 직접적인 영향을 주며 이후에 발생할 두통을 예방할 수 있습니다. multilib 타르볼은 가능하면 64비트 라이브러리를 사용하며 호환성이 필요하면 32비트 버전을 대신 사용합니다. 주요 설치 과정에 있어 최상의 옵션이며, 앞으로 다양한 개별 설정을 유연하게 처리할 수 있기 때문입니다. 프로파일을 쉽게 바꿀 수 있는 시스템을 원한다면 프로세서 아키텍처에 해당하는 multilib 타르볼 옵션을 다운로드해야합니다.
...
미러 목록에서 가까운 미러를 선택하십시오. HTTP 미러를 사용하는 걸로 충분합니다만, 다른 프로토콜로도 쓸 수 있습니다. releases/mips/autobuilds/ 디렉터리로 이동하십시오. 존재하는 모든 스테이지 파일이 나타납니다(아마도 각각의 하위 아키텍처에 있는 하위 디렉터리에 있을지도 모릅니다). 그 중 하나를 선택하고 `d` 를 눌러 다운로드하십시오.
...
For all non-official live images which offer gpg and wget in the live environment, refer to the earlier [Verifying the downloaded files](/wiki/Handbook:MIPS/Installation/Media/ko#Verifying_the_downloaded_files "Handbook:MIPS/Installation/Media/ko") section.
...
[← 디스크 준비](/wiki/Handbook:MIPS/Installation/Disks/ko "이전 내용") [처음](/wiki/Handbook:MIPS/ko "Handbook:MIPS/ko") [젠투 베이스 시스템 설치 →](/wiki/Handbook:MIPS/Installation/Base/ko "다음 내용")