...
- [6부팅](#.EB.B6.80.ED.8C.85)
  - [6.1기본: yaboot에서 설치 CD 부팅](#.EA.B8.B0.EB.B3.B8:_yaboot.EC.97.90.EC.84.9C_.EC.84.A4.EC.B9.98_CD_.EB.B6.80.ED.8C.85)
  - [6.2대안: 페가소스 시스템에서 설치 CD 부팅](#.EB.8C.80.EC.95.88:_.ED.8E.98.EA.B0.80.EC.86.8C.EC.8A.A4_.EC.8B.9C.EC.8A.A4.ED.85.9C.EC.97.90.EC.84.9C_.EC.84.A4.EC.B9.98_CD_.EB.B6.80.ED.8C.85)
  - [6.3대안: BootX에서 설치 CD 부팅](#.EB.8C.80.EC.95.88:_BootX.EC.97.90.EC.84.9C_.EC.84.A4.EC.B9.98_CD_.EB.B6.80.ED.8C.85)
  - [6.4키보드 배치 설정](#.ED.82.A4.EB.B3.B4.EB.93.9C_.EB.B0.B0.EC.B9.98_.EC.84.A4.EC.A0.95)
  - [6.5LiveGUI root access](#LiveGUI_root_access)
  - [6.6추가 하드웨어 설정](#.EC.B6.94.EA.B0.80_.ED.95.98.EB.93.9C.EC.9B.A8.EC.96.B4_.EC.84.A4.EC.A0.95)
  - [6.7선택: 사용자 계정](#.EC.84.A0.ED.83.9D:_.EC.82.AC.EC.9A.A9.EC.9E.90_.EA.B3.84.EC.A0.95)
  - [6.8선택: 설치 과정에 문서 보기](#.EC.84.A0.ED.83.9D:_.EC.84.A4.EC.B9.98_.EA.B3.BC.EC.A0.95.EC.97.90_.EB.AC.B8.EC.84.9C_.EB.B3.B4.EA.B8.B0)
    - [6.8.1TTY](#TTY)
    - [6.8.2GNU Screen](#GNU_Screen)
  - [6.9선택: SSH 데몬 시작](#.EC.84.A0.ED.83.9D:_SSH_.EB.8D.B0.EB.AA.AC_.EC.8B.9C.EC.9E.91)

## 하드웨어 요구 사항

시작하기 전에, 우선 ppc 장치에 젠투를 성공적으로 설치할 때 필요한 하드웨어 요구 사항을 하나하나 살펴보겠습니다.
...
소형 설치 CD는 install-ppc-minimal-<release>.iso 파일 이름을 지니고 있습니다.
...
스테이지 3 타르볼은 [공식 젠투 미러](https://www.gentoo.org/downloads/mirrors/) en중 어디서든 releases/ppc/autobuilds/ 위치에서 다운로드 할 수 있습니다. 스테이지 파일은 자주 업데이트하며, 설치 CD에는 없습니다.
...
2. ( **ppc/** 와 같은) 대상 아키텍처 관련 디렉터리를 선택하십시오
...
코드 **releases/ppc/autobuilds/current-iso/ 의 다운로드 파일 목록 예제**
...
위 예제에서 install-ppc-minimal-20141204.iso 파일이 소형 설치 CD 그 자체입니다. 그러나 보시다시피, 다른 관련 파일도 있습니다:
...
### 기본: yaboot에서 설치 CD 부팅

NewWorld 머신에서는 설치 CD를 CD-ROM에 넣고 시스템을 다시 부팅하십시오. 시스템 시작 알림 소리가 울리면 CD를 읽을 때까지 간단하게 `C` 를 누르고 기다리십시오.

설치 CD를 불러오고 나면 boot 프롬프트가 화면 바닥에 나타납니다.

일반 커널 ppc32를 제공합니다. 이 커널은 다중 CPU를 지원하도록 빌드했지만, 단일 프로세서 머신에서도 부팅합니다.

이 프롬프트에서 몇가지 커널 옵션을 입맛에 맞게 편집할 수 있습니다. 다음 표는 추가할 수 있는 부팅 옵션 몇가지를 나열했습니다:

Boot Option
Description
video
이 옵션은 nvidiafb, radeonfb, rivafb, atyfb, aty128, ofonly 중 제조사별 태그 한 가지를 따릅니다. 사용할 해상도 재생율과 색상 가지수는 이 태그를 따릅니다. 예를 들어 video=radeonfb:1280x1024@75-32 옵션은 ATI 라데온 프레임 버퍼를 선택하고 1280x1024 해상도와 75Hz 재생율 그리고 32비트 색상 표현 범위를 선택합니다. 선택한 내용이 정확하지 않고 기본값에 대해 동작하지 않는다면 video=ofonly 옵션이 가장 그럴싸하게 동작할 것입니다.
nol3
일부 PowerBook 기종의 레벨 3 캐시를 비활성화(최소한 17" 모니터에선 필요함)
dofirewire
하드디스크 같은 IEEE1394 (FireWire) 장치 지원을 활성화합니다.
dopcmcia
설치 과정에 PCMCIA 장치를 사용(PCMCIA 네트워크 카드)한다면 이 옵션을 활성화해야 합니다.
dosshd
sshd 시작. 설치 과정을 지켜보지 않을때 쓸만합니다.
passwd=foo
= 다음의 값을 루트 암호로 설정합니다. 원격 설치를 진행한다면 dosshd를 함께 사용하십시오.

위의 옵션을 활용하려면, boot 프롬프트에서 `ppc32` 를 입력하고 뒤이어 원하는 옵션을 입력하십시오. 아래 예제에서는, 커널에서 장치 개별 드라이버 대신 Open Firmware 프레임버퍼를 사용하도록 강제합니다.

`boot:` `ppc32 video=ofonly`

필요한 옵션이 없다면, 간단하게 ppc32를 입력하여 CD에서 완전한 젠투 리눅스 환경을 불러오십시오.

### 대안: 페가소스 시스템에서 설치 CD 부팅

페가소스에서는 간단하게 CD를 넣은 후 SmartFirmware 부팅 프롬프트에서 `cd /boot/menu` 를 입력하십시오.

`boot` `cd /boot/menu`

이 명령은 사용자가 여러가지 미리 설정한 비디오 설정 가운데 하나를 선택할 수 있게 하는 작은 부팅 메뉴를 엽니다. 다른 특별한 부팅 옵션은 위에서 설명한 Yaboot에서와 같이 명령줄에 붙일 수 있습니다. 예를 들자면:

`boot` `cd /boot/pegasos video=radeonfb:1280x1024@75 mem=256M`

기본 커널 옵션(뭔가 잘못됐을 경우)은 `console=ttyS0,115200 console=tty0 init=/linuxrc looptype=squashfs loop=/image.squashfs cdroot root=/dev/ram0` 로 미리 설정했습니다.

### 대안: BootX에서 설치 CD 부팅

OldWorld 맥 일부에서는 livecd를 사용할 수 없습니다. MacOS9 이전 버전을 활용하여 BootX 도구를 통해 리눅스 환경으로 부팅하는 방법이 가장 간단합니다.

먼저 BootX를 다운로드하고 아카이브를 해제하십시오. 아카이브를 해제한 내용에서 BootX 확장을 Extensions 폴더로 복사하고 BootX 앱 제어판을 제어판으로 복사하십시오. Extensions 폴더와 제어판은 MacOS System 폴더에 있습니다. 다음, _Linux Kernels_ 폴터를 System 폴더에 만들고 ppc32 커널을 CD에서 이 폴더로 복사하십시오. 마지막으로 ppc32.igz를 설치 CD boot 폴더에서 MacOS의 System 폴더로 복사하십시오.

BootX를 준비하려면 BootX 앱 제어판을 시작하십시오. 먼저 옵션 대화 상자를 선택하고 Use Speficied RAM Disk를 선택한 후 시스템 폴더에서 ppc32.igz를 선택하십시오. 초기 화면으로 계속 되돌아가서 램 디스크 크기가 최소한 32000인지 확인하십시오. 마지막으로, 커널 매개변수를 다음과 같이 설정하십시오:
...
위에 언급한 yaboot 섹션의 커널 매개 변수는 여기서도 적용할 수 있습니다. 해당 옵션을 위의 커널 매개 변수에 붙여보십시오.

설정이 올바른지 다시 확인하고 설정을 저장하십시오. 이 과정에서는 부팅하지 못하거나 뭔가 빠졌을 경우의 입력 내용을 저장합니다. 창 상단에 있는 Linux 버튼을 누르십시오. 모든 사항이 올바르게 진행되었다면, 설치 CD로 부팅합니다.

### 키보드 배치 설정

부팅하고 나면, 루트 ("#") 프롬프트가 현재 콘솔에 나타납니다. `Alt` + `F2`, `Alt` + `F3`, `Alt` + `F4` 를 눌러 다른 콘솔로 전환할 수 있습니다. `Alt` + `F1` 을 눌러 처음 콘솔로 돌아오십시오. 키보드 배치 때문에 애플 머신에서는 `Alt` + `fn` + `F#` 을 눌러야합니다.

비 US 키보드로 시스템에서 젠투를 설치할 때, 키보드의 키 배치를 불러오려면 loadkeys를 사용하십시오. 사용할 수 있는 키 배치를 보려면 `ls /usr/share/keymaps/i386` 명령을 실행하십시오.
...
`user $` `links https://wiki.gentoo.org/wiki/Handbook:PPC/ko`
...
sshd를 사용할 수 있게 하려면, 네트워크가 올바르게 동작해야 합니다. [네트워크 설정](/wiki/Handbook:PPC/Installation/Networking/ko "Handbook:PPC/Installation/Networking/ko") 장으로 계속 진행하십시오.

[← 젠투 리눅스 설치 정보](/wiki/Handbook:PPC/Installation/About/ko "이전 내용") [처음](/wiki/Handbook:PPC/ko "Handbook:PPC/ko") [네트워크 설정 →](/wiki/Handbook:PPC/Installation/Networking/ko "다음 내용")