...
If the init system being used is systemd, the partition UUIDs conform to the Discoverable Partition Specification as given in [Preparing the disks](/wiki/Handbook:PPC/Installation/Disks/ko "Handbook:PPC/Installation/Disks/ko"), and the system uses UEFI, then creating an fstab can be skipped, since systemd auto-mounts partitions that follow the spec.
...
ppc 분할 영역 예제에서, /boot/는 보통 ext2 파일 시스템을 쓰는 /dev/sda1 분할 영역입니다. 부팅 과정에 검사해야 하기 때문에 다음의 대용으로 적어내려가겠습니다:
...
For systems running OpenRC, a more detailed reference for network setup is available in the [advanced network configuration](/wiki/Handbook:PPC/Networking/Introduction/ko "Handbook:PPC/Networking/Introduction/ko") section, which is covered near the end of the handbook. Systems with more specific network needs may need to skip ahead, then return here to continue with the rest of the installation.
...
To set the system hostname for a system currently _running_ systemd, the hostnamectl utility may be used. During the installation process however, [systemd-firstboot](/wiki/Handbook:PPC/Installation/System/ko#Init_and_boot_configuration_systemd "Handbook:PPC/Installation/System/ko") command must be used instead (see later on in handbook).
...
[← 리눅스 커널 설정](/wiki/Handbook:PPC/Installation/Kernel/ko "이전 내용") [처음](/wiki/Handbook:PPC/ko "Handbook:PPC/ko") [시스템 도구 설치 →](/wiki/Handbook:PPC/Installation/Tools/ko "다음 내용")