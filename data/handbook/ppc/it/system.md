...
- [தமிழ்](/wiki/Handbook:PPC/Installation/System/ta "கையேடு:PPC/நிறுவல்/முறைமை (100% translated)")
...
Se il sistema di init che si sta usando è systemd, se le UUID delle partizioni rispettano la Discoverable Partition Specification (DPS) come specificato in [Preparare i dischi](/wiki/Handbook:PPC/Installation/Disks/it "Handbook:PPC/Installation/Disks/it"), e se il sistema usa UEFI, allora si può saltare la creazione dell'fstab poichè systemd monta automaticamente le partizioni che rispettano la specifica.
...
Nell'esempio di partizionamento per **ppc**, /boot/ è solitamente la partizione /dev/sda1, con xfs consigliato come filesystem. Esso necessita di essere controllato durante l'avvio, dunque dovremmo scrivere:
...
Sotto ecco un esempio di un file /etc/fstab per un disco formattato con le etichette disco GPT e le UUID impostate dal Discoverable Partition Specification (DPS) per un firmware UEFI:

FILE **`/etc/fstab`** **Esempio di fstab con etichette disco GPT e PARTUUID DPS**
...
Per impostare l'hostname di un sistema che al momento _esegue_ systemd, può essere usato il programma di utilità hostnamectl. Durante il processo di installazione comunque, il comando [systemd-firstboot](/wiki/Handbook:PPC/Installation/System/it#Init_and_boot_configuration_systemd "Handbook:PPC/Installation/System/it") deve essere invece usato (vedere più avanti nel manuale).
...
Informazioni più dettagliate sul funzionamento della rete, inclusi argomenti avanzati come accoppiamenti (bonding), interconnessioni (bridging), VLAN 802.1Q o reti wireless sono trattati nella sezione sulla [configurazione di rete avanzata](/wiki/Handbook:PPC/Networking/Introduction/it "Handbook:PPC/Networking/Introduction/it").
...
[← Configurare il kernel](/wiki/Handbook:PPC/Installation/Kernel/it "Previous part") [Home](/wiki/Handbook:PPC/it "Handbook:PPC/it") [Installare gli strumenti →](/wiki/Handbook:PPC/Installation/Tools/it "Next part")