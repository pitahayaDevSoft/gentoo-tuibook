...
- [3Selezionare il kernel](#Selezionare_il_kernel)
  - [3.1Kernel della distribuzione](#Kernel_della_distribuzione)
    ...
    - [3.1.2Installare un kernel della distribuzione](#Installare_un_kernel_della_distribuzione)
    - [3.1.3Aggiornare e fare pulizia](#Aggiornare_e_fare_pulizia)
    - [3.1.4Attività post-installazione/aggiornamento](#Attivit.C3.A0_post-installazione.2Faggiornamento)
      - [3.1.4.1Ricompilare manualmente l'initramfs o lo Unified Kernel Image](#Ricompilare_manualmente_l.27initramfs_o_lo_Unified_Kernel_Image)
  ...
  - [3.3Compilare e installare](#Compilare_e_installare)
...
Installare certi pacchetti firmware spesso richiede l'accettazione di licenze associate al firmware. Se necessario, visitare la [sezione di gestione delle licenze](/wiki/Handbook:PPC/Working/Portage/it#Licenses "Handbook:PPC/Working/Portage/it") del manuale per aiuto sull'accettazione delle licenze.
...
[Approccio completamente automatico: kernel della distribuzione](/wiki/Handbook:PPC/Installation/Kernel/it#Distribution_kernels "Handbook:PPC/Installation/Kernel/it")Un [kernel della distribuzione](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel") è usato per configurare, compilare automaticamente ed installare il kernel Linux, i suoi moduli associati e (facoltativamente, ma per impostazione predefinita) un file initramfs. Gli aggiornamenti futuri del kernel sono completamente automatizzati poichè se ne occupa il gestore dei pacchetti, come qualsiasi altro pacchetto del sistema. E' possibile [fornire un file di configurazione del kernel su misura](/wiki/Project:Distribution_Kernel#Modifying_kernel_configuration "Project:Distribution Kernel") se è necessaria la personalizzazione. Questo è il procedimento meno complesso ed è perfetto per i nuovi utenti di Gentoo per via del suo essere pronto all'uso e del suo offrire una interazione minima da parte dell'amministratore di sistema.[Approccio manuale](/wiki/Handbook:PPC/Installation/Kernel/it#Alternative:_Manual_configuration "Handbook:PPC/Installation/Kernel/it")I nuovi sorgenti del kernel sono installati attraverso il gestore dei pacchetti di sistema. Il kernel viene configurato, compilato e installato manualmente usando i comandi eselect kernel e una sfilza di make. Gli aggiornamenti futuri del kernel ripetono il procedimento manuale di configurare , compilare e installare i file del kernel. Questo è il procedimento più complesso ma offre il massimo controllo sul processo di aggiornamento del kernel.
...
I _[kernel della distribuzione](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel")_, come [sys-kernel/gentoo-kernel](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel) e [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin), di norma, si aspettano di essere installati insieme ad [initramfs](/wiki/Handbook:PPC/Installation/Kernel#Initramfs.2Fit "Handbook:PPC/Installation/Kernel"). Prima di fare l'emerge per installare il kernel gli utenti dovrebbero accertarsi che [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) sia stato configurato per utilizzare un generatore di initramfs (per esempio [Dracut](/wiki/Dracut "Dracut")) come descritto nella sezione [installkernel](/wiki/Handbook:PPC/Installation/Kernel#Initramfs.2Fit "Handbook:PPC/Installation/Kernel").
...
Dopo aver installato con successo il kernel della distribuzione, è ora il momento di procedere con la prossima sezione: [Configurare il sistema](/wiki/Handbook:PPC/Installation/System/it "Handbook:PPC/Installation/System/it").

### In alternativa: Configurazione manuale

#### Installare e configurare i sorgenti del kernel

Quando si compila manualmente il kernel per i sistemi basati su ppc, Gentoo consiglia il pacchetto [sys-kernel/gentoo-sources](https://packages.gentoo.org/packages/sys-kernel/gentoo-sources).
...
Intanto si prega di eseguire l'installazione di [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin) come descritto in [kernel della distribuzione](/wiki/Handbook:PPC/Installation/Kernel#Distribution_kernels.2Fit "Handbook:PPC/Installation/Kernel").
...
Ora è tempo di decidere se la firma dei moduli è necessaria nei passaggi elencati [qui](/wiki/Handbook:PPC/Installation/Kernel#Optional:_Signed_kernel_modules_2.2Fit "Handbook:PPC/Installation/Kernel").

Altrimenti, proseguire con la compilazione come descritta [qui](/wiki/Handbook:PPC/Installation/Kernel#Compiling_and_installing.2Fit "Handbook:PPC/Installation/Kernel").
...
Quando il kernel è configurato, è ora di compilarlo e installarlo. Uscire dal menù di configurazione ed eseguire i seguenti comandi:

`root #` `make && make modules_install`

**Nota**

E' possibile abilitare compilazioni parallele usando make -j N con N _numero_ intero delle operazioni parallele che al processo di compilazione è permesso lanciare. Ciò è similare alle istruzioni di prima su /etc/portage/make.conf, con la variabile MAKEOPTS.

Quando il kernel ha terminato la compilazione, copiare l'immagine del kernel in /boot/. Di ciò si occupa il comando make install. Quando si usa BootX per avviare, il kernel sarà copiato in seguito.

`root #` `make install`

Questo comando copierà l'immagine del kernel in /boot. Se [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) è installato eseguirà invece /sbin/installkernel a cui delegherà l'installazione del kernel. Invece di copiare semplicemente il kernel in /boot, [Installkernel](/wiki/Installkernel "Installkernel") installa ogni kernel col suo numero di versione nel nome del file. Inoltre, installkernel fornisce un infrastruttura per compiere automaticamente varie operazioni relative all'installazione del kernel, come: generare un [initramfs](/wiki/Initramfs "Initramfs"), costruire un [UKI](/wiki/Unified_Kernel_Image "Unified Kernel Image") e aggiornare la configurazione del [programma d'avvio](/wiki/Bootloader "Bootloader").

Continuare l'installazione con [configurare il sistema](/wiki/Handbook:PPC/Installation/System/it "Handbook:PPC/Installation/System/it").

[← Installare il sistema base](/wiki/Handbook:PPC/Installation/Base/it "Previous part") [Home](/wiki/Handbook:PPC "Handbook:PPC") [Configurare il sistema →](/wiki/Handbook:PPC/Installation/System/it "Next part")