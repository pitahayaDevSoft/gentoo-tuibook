...
- [தமிழ்](/wiki/Handbook:PPC/Installation/Base/ta "கையேடு:PPC/நிறுவல்/அடிப்படை (100% translated)")
...
Si la instalación de Gentoo se interrumpe en algún momento posterior, _debería_ ser posible 'continuarla' desde este paso. ¡No es necesario particionar los discos otra vez! Simplemente [monte la partición raíz](/wiki/Handbook:PPC/Installation/Disks/es#Mounting_the_root_partition "Handbook:PPC/Installation/Disks/es") y siga los pasos anteriores comenzando desde [copiar la información DNS](/wiki/Handbook:PPC/Installation/Base/es#Copy_DNS_info "Handbook:PPC/Installation/Base/es") hasta entrar en el nuevo entorno. Esto también se utiliza para arreglar problemas con el cargador de arranque. Mas información en el artículo [chroot](/wiki/Chroot/es "Chroot/es").
...
Después de revisar los perfiles disponibles para la arquitectura ppc, los usuarios pueden elegir usar un perfil diferente para el sistema:
...
La configuración del repositorio para un binhost se encuentra en el directorio /etc/portage/binrepos.conf/ de Portage, que funciona de manera similar a la configuración mencionada en la sección [repositorio de ebuilds de Gentoo](/wiki/Handbook:PPC/Installation/Base/es#Gentoo_ebuild_repository "Handbook:PPC/Installation/Base/es").

Al definir un host de binarios, hay dos aspectos importantes a considerar:

1. La arquitectura y perfiles dentro del valor `sync-uri` _sí_ importan y deben alinearse con la arquitectura de ordenadores respectiva ( **ppc** en este caso) y el perfil del sistema seleccionado en la sección [Elegir el perfil correcto](/wiki/Handbook:PPC/Installation/Base/es#Choosing_the_right_profile "Handbook:PPC/Installation/Base/es").
2. Seleccionar un mirror rápido y geográficamente cercano generalmente acortará el tiempo de recuperación. Revise la herramienta mirrorselect mencionada en la sección [Opcional: Seleccionar mirrors](/wiki/Handbook:PPC/Installation/Base/es#Gentoo_ebuild_repository "Handbook:PPC/Installation/Base/es") o revise el [lista en línea de mirrors](https://www.gentoo.org/downloads/mirrors/) donde se pueden descubrir los valores de URL.
...
Las funciones adicionales de Portage se analizarán en el [siguiente capítulo](/wiki/Handbook:PPC/Working/Features/es#Portage_features "Handbook:PPC/Working/Features/es") del manual.
...
Los lectores que agregaron un servidor de binarios [anteriormente](/wiki/Handbook:PPC/Installation/Base/es#Optional:_Adding_a_binary_package_host "Handbook:PPC/Installation/Base/es") pueden agregar --getbinpkg (o -g) para obtener paquetes del servidoe de binarios en lugar de compilarlos:
...
[← Instalar el archivo de Stage](/wiki/Handbook:PPC/Installation/Stage/es "Previous part") [Inicio](/wiki/Handbook:PPC/es "Handbook:PPC/es") [Configurar el núcleo →](/wiki/Handbook:PPC/Installation/Kernel/es "Next part")