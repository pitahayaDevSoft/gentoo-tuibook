...
Bitte wählen Sie in der Spiegel-Liste einen Spiegel in Ihrer Nähe. Für gewöhnlich genügen HTTP Spiegel, andere Protokolle stehen aber auch zur Verfügung. Gehen Sie in das Verzeichnis releases/ppc/autobuilds/. Dort werden alle verfügbaren Stage Tar-Archive angezeigt (sie können in Unterverzeichnissen gespeichert sein, benannt nach den einzelnen Sub-Architekturen). Wählen Sie eines aus und drücken Sie `d` zum Download.

Nachdem Sie das Stage Tar-Archiv erfolgreich heruntergeladen haben, können Sie die Integrität des Tar-Archivs verifizieren und den Inhalt validieren. Wie das geht, steht im [folgenden Abschnitt](/wiki/Handbook:PPC/Installation/Stage/de#Verifying_and_validating "Handbook:PPC/Installation/Stage/de").

Wenn Sie kein Interesse an einer Überprüfung und Validierung des Stage Tar-Archivs haben, können Sie jetzt `q` drücken, um den Browser zu beenden. Springen Sie danach zu dem Abschnitt [Stage Tar-Archiv entpacken](/wiki/Handbook:PPC/Installation/Stage/de#Unpacking_the_stage_tarball "Handbook:PPC/Installation/Stage/de").
...
For all non-official live images which offer gpg and wget in the live environment, refer to the earlier [Verifying the downloaded files](/wiki/Handbook:PPC/Installation/Media/de#Verifying_the_downloaded_files "Handbook:PPC/Installation/Media/de") section.
...
`root #` `gpg --verify stage3-ppc-<release>-<init>.tar.xz.DIGEST
`
...
Nachdem nun das Stage Tar-Archiv ausgepackt ist, geht es weiter mit dem Schritt: [Compiler-Optionen konfigurieren](/wiki/Handbook:PPC/Installation/Stage/de#Configuring_compile_options "Handbook:PPC/Installation/Stage/de").
...
[← Vorbereiten der Festplatte(n)](/wiki/Handbook:PPC/Installation/Disks/de "Previous part") [Anfang](/wiki/Handbook:PPC/de "Handbook:PPC/de") [Installation des Basissystems →](/wiki/Handbook:PPC/Installation/Base/de "Next part")