...
- [3Kernel selection](#Kernel_selection)
  ...
  - [3.3Compiling and installing](#Compiling_and_installing)
...
Installing certain firmware packages often requires accepting the associated firmware licenses. If necessary, visit the [license handling section](/wiki/Handbook:PPC/Working/Portage/pl#Licenses "Handbook:PPC/Working/Portage/pl") of the Handbook for help on accepting licenses.
...
[Full automation approach: Distribution kernels](/wiki/Handbook:PPC/Installation/Kernel/pl#Distribution_kernels "Handbook:PPC/Installation/Kernel/pl")A [Distribution Kernel](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel") is used to configure, automatically build, and install the Linux kernel, its associated modules, and (optionally, but enabled by default) an initramfs file. Future kernel updates are fully automated since they are handled through the package manager, just like any other system package. It is possible [provide a custom kernel configuration file](/wiki/Project:Distribution_Kernel#Modifying_kernel_configuration "Project:Distribution Kernel") if customization is necessary. This is the least involved process and is perfect for new Gentoo users due to it working out-of-the-box and offering minimal involvement from the system administrator.[Manual approach](/wiki/Handbook:PPC/Installation/Kernel/pl#Alternative:_Manual_configuration "Handbook:PPC/Installation/Kernel/pl")New kernel sources are installed via the system package manager. The kernel is manually configured, built, and installed using the eselect kernel and a slew of make commands. Future kernel updates repeat the manual process of configuring, building, and installing the kernel files. This is the most involved process, but offers maximum control over the kernel update process.
...
Kernel installation tasks such as copying the kernel image to /boot or the [EFI System Partition](/wiki/EFI_System_Partition "EFI System Partition"), generating an [initramfs](/wiki/Initramfs "Initramfs") and/or [Unified Kernel Image](/wiki/Unified_Kernel_Image "Unified Kernel Image"), updating bootloader configuration, can be automated with [installkernel](/wiki/Installkernel "Installkernel"). Users may wish to configure and install [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) before proceeding. See the [Kernel installation section below](/wiki/Handbook:PPC/Installation/Kernel#Kernel_installation.2Fpl "Handbook:PPC/Installation/Kernel") for more more information.
...
_[Distribution Kernels](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel")_, such as [sys-kernel/gentoo-kernel](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel) and [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin), by default, expect to be installed alongside an [initramfs](/wiki/Handbook:PPC/Installation/Kernel#Initramfs.2Fpl "Handbook:PPC/Installation/Kernel"). Before running emerge to install the kernel users should ensure that [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) has been configured to utilize an initramfs generator (for example [Dracut](/wiki/Dracut "Dracut")) as described in the [installkernel section](/wiki/Handbook:PPC/Installation/Kernel#Initramfs.2Fpl "Handbook:PPC/Installation/Kernel").
...
After installing the Distribution Kernel successfully, it is now time to proceed to the next section: [Configuring the system](/wiki/Handbook:PPC/Installation/System/pl "Handbook:PPC/Installation/System/pl").
...
For now please follow installing [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin) via [Distribution\_kernels](/wiki/Handbook:PPC/Installation/Kernel#Distribution_kernels.2Fpl "Handbook:PPC/Installation/Kernel").
...
Now it's time to decide if modules signing is required in the steps listed in [here](/wiki/Handbook:PPC/Installation/Kernel#Optional:_Signed_kernel_modules_2.2Fpl "Handbook:PPC/Installation/Kernel")

If not, proceed to building described [here](/wiki/Handbook:PPC/Installation/Kernel#Compiling_and_installing.2Fpl "Handbook:PPC/Installation/Kernel")
...
Continue the installation with [Configuring the system](/wiki/Handbook:PPC/Installation/System/pl "Handbook:PPC/Installation/System/pl").

[← Installing base system](/wiki/Handbook:PPC/Installation/Base "Previous part") [Home](/wiki/Handbook:PPC "Handbook:PPC") [Configuring the system →](/wiki/Handbook:PPC/Installation/System "Next part")