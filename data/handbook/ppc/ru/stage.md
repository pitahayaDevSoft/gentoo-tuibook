...
В списке зеркал выберите зеркало, которое находится рядом. Обычно достаточно HTTP-зеркала, но другие протоколы также доступны. Перейдите в каталог releases/ppc/autobuilds/. Там отображаются все доступные stage-файлы (они могут находиться в подкаталогах с названиями отдельных субархитектур). Выберите нужный и нажмите `d` для скачивания.

После завершения скачивания можно проверить целостность и достоверность содержимого файла stage. Если вам это интересно, перейдите к [следующему разделу](/wiki/Handbook:PPC/Installation/Stage/ru#Verifying_and_validating "Handbook:PPC/Installation/Stage/ru").

Тем, кому не интересно проверять архив stage, могут закрыть браузер в командной строке с помощью клавиши `q` и сразу перейти к разделу [Распаковка файла stage](/wiki/Handbook:PPC/Installation/Stage/ru#Unpacking_the_stage_file "Handbook:PPC/Installation/Stage/ru").
...
Для неофициальных образов, которые содержат утилиты gpg и wget, см. предыдущий раздел [Проверка скачанных файлов](/wiki/Handbook:PPC/Installation/Media/ru#Verifying_the_downloaded_files "Handbook:PPC/Installation/Media/ru").
...
Теперь, когда stage распакован, перейдём к [настройке параметров компиляции](/wiki/Handbook:PPC/Installation/Stage/ru#Configuring_compile_options "Handbook:PPC/Installation/Stage/ru").
...
[← Подготовка дисков](/wiki/Handbook:PPC/Installation/Disks/ru "Предыдущая часть") [К содержанию](/wiki/Handbook:PPC/ru "Handbook:PPC/ru") [Установка базовой системы Gentoo →](/wiki/Handbook:PPC/Installation/Base/ru "Следующая часть")