...
시작하기 전에, 우선 sparc 장치에 젠투를 성공적으로 설치할 때 필요한 하드웨어 요구 사항을 하나하나 살펴보겠습니다.
...
소형 설치 CD는 install-sparc-minimal-<release>.iso 파일 이름을 지니고 있습니다.
...
스테이지 3 타르볼은 [공식 젠투 미러](https://www.gentoo.org/downloads/mirrors/) en중 어디서든 releases/sparc/autobuilds/ 위치에서 다운로드 할 수 있습니다. 스테이지 파일은 자주 업데이트하며, 설치 CD에는 없습니다.
...
2. ( **sparc/** 와 같은) 대상 아키텍처 관련 디렉터리를 선택하십시오
...
코드 **releases/sparc/autobuilds/current-iso/ 의 다운로드 파일 목록 예제**
...
위 예제에서 install-sparc-minimal-20141204.iso 파일이 소형 설치 CD 그 자체입니다. 그러나 보시다시피, 다른 관련 파일도 있습니다:
...
젠투 설치 CD를 CD-ROM에 넣고 시스템을 부팅하십시오. 시작 과정에서 `Stop` + `A` 를 눌러 OpenBootPROM(OBP)에 진입하십시오. OBP로 들어가서 CD-ROM으로 부팅하십시오:

`ok` `boot cdrom`

(설치 CD의) SILO 부팅 관리자를 시작합니다. 도움말을 보려면 `Enter` 를 치십시오. _gentoo_ 를 입력하고 `Enter` 를 눌러 시스템 부팅을 계속하십시오:

`boot:` `gentoo`

설치 CD를 부팅하면 루트 ("#") 프롬프트가 현재 콘솔에 나타납니다. 직렬 콘솔(ttyS0)에도 루트 프롬프트가 뜹니다.
...
`user $` `links https://wiki.gentoo.org/wiki/Handbook:SPARC/ko`
...
sshd를 사용할 수 있게 하려면, 네트워크가 올바르게 동작해야 합니다. [네트워크 설정](/wiki/Handbook:SPARC/Installation/Networking/ko "Handbook:SPARC/Installation/Networking/ko") 장으로 계속 진행하십시오.

[← 젠투 리눅스 설치 정보](/wiki/Handbook:SPARC/Installation/About/ko "이전 내용") [처음](/wiki/Handbook:SPARC/ko "Handbook:SPARC/ko") [네트워크 설정 →](/wiki/Handbook:SPARC/Installation/Networking/ko "다음 내용")