...
- [5Rendszerindítás (bootolás)](#Rendszerind.C3.ADt.C3.A1s_.28bootol.C3.A1s.29)
  - [5.1A telepítő CD bootolása](#A_telep.C3.ADt.C5.91_CD_bootol.C3.A1sa)
...
Mielőtt folytatná a telepítést, az Ön számítógépének teljesítenie kell a minimális hardverkövetelményeket a Gentoo **sparc** rendszerarchitektúrára való sikeres telepítés érdekében.
...
Az úgynevezett stage 3 (3. fokozat) tar fájlok letölthetőek az internetről a releases/sparc/autobuilds/ oldalról bármelyik [hivatalos Gentoo tükörszerverről](https://www.gentoo.org/downloads/mirrors/). A stage fájlokat gyakran frissítik, ezért azok nincsenek benne a hivatalos Live ISO-képfájlokban.
...
3. Válassza ki az Önnek megfelelő célarchitektúra könyvtárat (úgy mint **sparc/**).
...
`user $` `gpg --verify install-sparc-minimal-20240107T170309Z.iso.asc`

```
gpg: assuming signed data in 'install-sparc-minimal-20240107T170309Z.iso'
...
### A telepítő CD bootolása

Helyezze be a Gentoo telepítő CD-t a CD-ROM meghajtóba, és indítsa el a rendszert. Az indítás során nyomja meg a `Stop` \+ `A` billentyűkombinációt az OpenBootPROM (OBP) eléréséhez. Az OBP-ben lépjen tovább a CD-ROM-ról történő indításhoz:

`ok` `boot cdrom`

Ezután elindul a SILO boot manager (a telepítő CD-n). Nyomja meg a `Enter` gombot további segítségért. Írja be, hogy _gentoo_, majd nyomja meg a `Enter` gombot a rendszer indításának folytatásához:

`boot:` `gentoo`

Miután a Telepítő CD elindult, a root ("#") prompt jelenik meg az aktuális konzolon. Emellett a soros konzolon (ttyS0) is megjelenik egy root prompt.
...
Az sshd szolgáltatás használatához a hálózatnak megfelelően kell működnie. Folytassa a [Hálózat beállítása](/wiki/Handbook:SPARC/Installation/Networking/hu "Handbook:SPARC/Installation/Networking/hu") című fejezettel.

[← A telepítésről](/wiki/Handbook:SPARC/Installation/About/hu "Previous part") [Kezdőlap](/wiki/Handbook:SPARC/hu "Handbook:SPARC/hu") [Hálózat beállítása →](/wiki/Handbook:SPARC/Installation/Networking/hu "Next part")