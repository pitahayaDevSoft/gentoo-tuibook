...
2M
none
[BIOS boot partition](/wiki/Handbook:X86/Blocks/Disks#What_is_the_BIOS_boot_partition.3F.2Fpl "Handbook:X86/Blocks/Disks")/dev/sda2(swap)
RAM size \* 2
none
Swap partition
/dev/sda3ext4
...
Now that the partitions have been created, it is time to place a filesystem on them. In the next section the various file systems that Linux supports are described. Readers that already know which filesystem to use can continue with [Applying a filesystem to a partition](/wiki/Handbook:SPARC/Installation/Disks/pl#Applying_a_filesystem_to_a_partition "Handbook:SPARC/Installation/Disks/pl"). The others should read on to learn about the available filesystems...
...
Later in the instructions, the proc filesystem (a virtual interface with the kernel) as well as other kernel pseudo-filesystems will be mounted. But first [the Gentoo stage file](/wiki/Handbook:SPARC/Installation/Stage/pl "Handbook:SPARC/Installation/Stage/pl") must be extracted.
