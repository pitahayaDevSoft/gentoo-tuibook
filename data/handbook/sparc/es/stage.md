...
Seleccione un servidor réplica cercano. Normalmente bastará con los servidores HTTP, sin embargo también están disponibles otros protocolos. Entre en el directorio releases/sparc/autobuilds/. En él deberían aparecer todos los archivos de stage disponibles (quizá almacenados en subdirectorios con el nombre de cada subarquitectura). Seleccione uno y pulse `d` para descargarlo.

Una vez haya finalizada la descarga del archivo de stage, es posible verificar la integridad y validar los contenidos del archivo de stage. Los interesados pueden ir a la [siguiente sección](/wiki/Handbook:SPARC/Installation/Stage/es#Verifying_and_validating "Handbook:SPARC/Installation/Stage/es").

Aquellos que no estén interesados en verificar y validar el archivo de stage pueden cerrar el navegador de línea de comandos presionando `q` e ir directamente a la sección [Descomprimir el archivo de stage](/wiki/Handbook:SPARC/Installation/Stage/es#Unpacking_the_stage_file "Handbook:SPARC/Installation/Stage/es").
...
`root #` `gpg --verify stage3-sparc-<release>-<init>.tar.xz.asc
`

`root #` `gpg --verify stage3-sparc-<release>-<init>.tar.xz.DIGESTS
`

`root #` `gpg --verify stage3-sparc-<release>-<init>.tar.xz.sha256
`
...
Ahora que el archivo stage está desempaquetado, continúe con [Configurara las opciones de compilación](/wiki/Handbook:SPARC/Installation/Stage/es#Configuring_compile_options "Handbook:SPARC/Installation/Stage/es").
...
COMMON_CFLAGS="-O2 -mcpu=ultrasparc -pipe"
...
[← Preparando los discos](/wiki/Handbook:SPARC/Installation/Disks/es "Previous part") [Inicio](/wiki/Handbook:SPARC/es "Handbook:SPARC/es") [Instalar el sistema base →](/wiki/Handbook:SPARC/Installation/Base/es "Next part")