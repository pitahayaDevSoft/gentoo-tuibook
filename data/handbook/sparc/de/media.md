...
Bevor Sie mit dem Installationsprozess fortfahren, sollten Sie die Mindestanforderungen an die Hardware erfüllen, um Gentoo erfolgreich für die Systemarchitektur **sparc** zu installieren.
...
[Vorbereiten der Festplatte(n)](/wiki/Handbook:SPARC/Installation/Disks/de "Handbook:SPARC/Installation/Disks/de").
...
Die Minimal-Installations-CDs werden im folgendem Format benannt: install-sparc-minimal-<release>.iso.
...
Stage3-Dateien können aus dem Verzeichnis releases/sparc/autobuilds/ von einem der [offiziellen Gentoo Spiegel-Server (Mirror-Server)](https://www.gentoo.org/downloads/mirrors/) heruntergeladen werden. Die Stage3-Dateien werden häufig aktualisiert und sind daher nicht auf den offiziellen Installations-Images enthalten.
...
2. Gehen Sie in das Verzeichnis für die richtige Architektur, z.B. **sparc/**
...
CODE **Beispiel-Listing des Verzeichnisses releases/sparc/autobuilds/current-iso/**
...
In dem obigen Beispiel ist die Datei install-sparc-minimal-20141204.iso die Minimal-Installations-CD. Wie Sie sehen, existieren aber auch noch weitere, zugehörige Dateien:
...
Legen Sie die Gentoo Installations-CD in das CD-ROM Laufwerk und booten Sie das System. Drücken Sie während des Startvorgangs `Stop` \+ `A` um in das OpenBootPROM (OBP) zu gelangen. Wenn Sie sich im OBP befinden, booten sie von der CD-ROM:

`ok` `boot cdrom`

Der SILO Bootmanager (auf der Installations-CD) ist dann gestartet. Drücken Sie `Enter` für weitere Hilfe. Geben Sie _gentoo_ ein und drücken Sie `Enter` um mit dem Booten des Systems fortzufahren:

`boot:` `gentoo`

Sobald die Installations-CD gebootet ist, wird ein root ("#") Prompt auf der aktuellen Konsole angezeigt. Auf der seriellen Konsole gibt es ebenfalls einen root Prompt (ttyS0).
...
`user $` `links https://wiki.gentoo.org/wiki/Handbook:SPARC/de`
...
Um sshd nutzen zu können, müssen Sie zunächst das Netzwerk einrichten. Setzen Sie die Installation mit der [Konfiguration des Netzwerks](/wiki/Handbook:SPARC/Installation/Networking/de "Handbook:SPARC/Installation/Networking/de") fort.

[← Über die Installation](/wiki/Handbook:SPARC/Installation/About/de "Previous part") [Anfang](/wiki/Handbook:SPARC/de "Handbook:SPARC/de") [Konfiguration des Netzwerks →](/wiki/Handbook:SPARC/Installation/Networking/de "Next part")