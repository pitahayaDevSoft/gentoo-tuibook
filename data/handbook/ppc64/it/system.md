...
Se il sistema di init che si sta usando è systemd, se le UUID delle partizioni rispettano la Discoverable Partition Specification (DPS) come specificato in [Preparare i dischi](/wiki/Handbook:PPC64/Installation/Disks/it "Handbook:PPC64/Installation/Disks/it"), e se il sistema usa UEFI, allora si può saltare la creazione dell'fstab poichè systemd monta automaticamente le partizioni che rispettano la specifica.
...
Nell'esempio di partizionamento per **ppc64**, /boot/ è solitamente la partizione /dev/sda1, con xfs consigliato come filesystem. Esso necessita di essere controllato durante l'avvio, dunque dovremmo scrivere:
...
Sotto un esempio di un file /etc/fstab per un disco formattato con un etichetta disco GPT e gli UUID configurati dal Discoverable Partition Specification (DPS) per il firmware UEFI:

FILE **`/etc/fstab`** **Esempio di fstab con etichetta disco GPT e DPS PARTUUID**

```
# Adattare per formattazioni diverse e partizioni aggiuntive create nel passaggio "Preparare i dischi".
# Questo esempio mostra un etichetta disco GPT con UUID impostati dal Discoverable Partition Specification (DSP):
...
Per impostare l'hostname di un sistema che al momento _esegue_ systemd, può essere usato il programma di utilità hostnamectl. Durante il processo di installazione comunque, il comando [systemd-firstboot](/wiki/Handbook:PPC64/Installation/System/it#Init_and_boot_configuration_systemd "Handbook:PPC64/Installation/System/it") deve essere invece usato (vedere più avanti nel manuale).
...
Informazioni più dettagliate sul funzionamento della rete, inclusi argomenti avanzati come accoppiamenti (bonding), interconnessioni (bridging), VLAN 802.1Q o reti wireless sono trattati nella sezione sulla [configurazione di rete avanzata](/wiki/Handbook:PPC64/Networking/Introduction/it "Handbook:PPC64/Networking/Introduction/it").
...
[← Configurare il kernel](/wiki/Handbook:PPC64/Installation/Kernel/it "Previous part") [Home](/wiki/Handbook:PPC64/it "Handbook:PPC64/it") [Installare gli strumenti →](/wiki/Handbook:PPC64/Installation/Tools/it "Next part")