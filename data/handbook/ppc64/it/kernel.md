...
- [3Selezionare il kernel](#Selezionare_il_kernel)
  ...
  - [3.2In alternativa: Configurazione manuale](#In_alternativa:_Configurazione_manuale)
    ...
    - [3.2.3Compilare e installare](#Compilare_e_installare)
...
Installare certi pacchetti firmware spesso richiede l'accettazione di licenze associate al firmware. Se necessario, visitare la [sezione di gestione delle licenze](/wiki/Handbook:PPC64/Working/Portage/it#Licenses "Handbook:PPC64/Working/Portage/it") del manuale per aiuto sull'accettazione delle licenze.
...
[Approccio completamente automatico: kernel della distribuzione](/wiki/Handbook:PPC64/Installation/Kernel/it#Distribution_kernels "Handbook:PPC64/Installation/Kernel/it")Un [kernel della distribuzione](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel") è usato per configurare, compilare automaticamente ed installare il kernel Linux, i suoi moduli associati e (facoltativamente, ma per impostazione predefinita) un file initramfs. Gli aggiornamenti futuri del kernel sono completamente automatizzati poichè se ne occupa il gestore dei pacchetti, come qualsiasi altro pacchetto del sistema. E' possibile [fornire un file di configurazione del kernel su misura](/wiki/Project:Distribution_Kernel#Modifying_kernel_configuration "Project:Distribution Kernel") se è necessaria la personalizzazione. Questo è il procedimento meno complesso ed è perfetto per i nuovi utenti di Gentoo per via del suo essere pronto all'uso e del suo offrire una interazione minima da parte dell'amministratore di sistema.[Approccio manuale](/wiki/Handbook:PPC64/Installation/Kernel/it#Alternative:_Manual_configuration "Handbook:PPC64/Installation/Kernel/it")I nuovi sorgenti del kernel sono installati attraverso il gestore dei pacchetti di sistema. Il kernel viene configurato, compilato e installato manualmente usando i comandi eselect kernel e una sfilza di make. Gli aggiornamenti futuri del kernel ripetono il procedimento manuale di configurare , compilare e installare i file del kernel. Questo è il procedimento più complesso ma offre il massimo controllo sul processo di aggiornamento del kernel.
...
I _[kernel della distribuzione](/wiki/Project:Distribution_Kernel "Project:Distribution Kernel")_, come [sys-kernel/gentoo-kernel](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel) e [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin), di norma, si aspettano di essere installati insieme ad [initramfs](/wiki/Handbook:PPC64/Installation/Kernel#Initramfs.2Fit "Handbook:PPC64/Installation/Kernel"). Prima di fare l'emerge per installare il kernel gli utenti dovrebbero accertarsi che [sys-kernel/installkernel](https://packages.gentoo.org/packages/sys-kernel/installkernel) sia stato configurato per utilizzare un generatore di initramfs (per esempio [Dracut](/wiki/Dracut "Dracut")) come descritto nella sezione [installkernel](/wiki/Handbook:PPC64/Installation/Kernel#Initramfs.2Fit "Handbook:PPC64/Installation/Kernel").
...
Dopo aver installato con successo il kernel della distribuzione, è ora il momento di procedere con la prossima sezione: [Configurare il sistema](/wiki/Handbook:PPC64/Installation/System/it "Handbook:PPC64/Installation/System/it").

### In alternativa: Configurazione manuale

#### Installare e configurare i sorgenti del kernel

Quando si compila manualmente il kernel per i sistemi basati su ppc64, Gentoo consiglia il pacchetto [sys-kernel/gentoo-sources](https://packages.gentoo.org/packages/sys-kernel/gentoo-sources).
...
Intanto si prega di eseguire l'installazione di [sys-kernel/gentoo-kernel-bin](https://packages.gentoo.org/packages/sys-kernel/gentoo-kernel-bin) come descritto in [kernel della distribuzione](/wiki/Handbook:PPC64/Installation/Kernel#Distribution_kernels.2Fit "Handbook:PPC64/Installation/Kernel").
...
Ora è tempo di decidere se la firma dei moduli è necessaria nei passaggi elencati [qui](/wiki/Handbook:PPC64/Installation/Kernel#Optional:_Signed_kernel_modules_2.2Fit "Handbook:PPC64/Installation/Kernel").

Altrimenti, proseguire con la compilazione come descritta [qui](/wiki/Handbook:PPC64/Installation/Kernel#Compiling_and_installing.2Fit "Handbook:PPC64/Installation/Kernel").
...
Ora che il kernel è configurato, è ora di compilarlo e installarlo. Uscire dalla configurazione e iniziare il processo di compilazione:

`root #` `make && make modules_install`

**Nota**

E' possibile abilitare compilazioni parallele usando make -j N dove N è il _numero_ intero di operazioni parallele che al processo di compilazione è consentito lanciare. Ciò è similare alle istruzioni precedenti riguardo /etc/portage/make.conf, con la variabile MAKEOPTS.

Quando il kernel è stato compilato, copiarne l'immagine in /boot/. Ciò viene gestito dal comando make install:
...
Continuare l'installazione con [configurare il sistema](/wiki/Handbook:PPC64/Installation/System/it "Handbook:PPC64/Installation/System/it").

[← Installare il sistema base](/wiki/Handbook:PPC64/Installation/Base/it "Previous part") [Home](/wiki/Handbook:PPC64 "Handbook:PPC64") [Configurare il sistema →](/wiki/Handbook:PPC64/Installation/System/it "Next part")