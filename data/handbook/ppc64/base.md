...
If the Gentoo installation is interrupted anywhere after this point, it _should_ be possible to 'resume' the installation at this step. There is no need to re-partition the disks again! Simply check the date and time, [mount the root partition](/wiki/Handbook:PPC64/Installation/Disks#Mounting_the_root_partition "Handbook:PPC64/Installation/Disks") and run the steps above starting with [copying the DNS info](/wiki/Handbook:PPC64/Installation/Base#Copy_DNS_info "Handbook:PPC64/Installation/Base") to re-enter the working environment. This is also useful for fixing bootloader issues. More information can be found in the [chroot](/wiki/Chroot "Chroot") article.
...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:PPC64/Installation/Base#Gentoo_ebuild_repository "Handbook:PPC64/Installation/Base") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **ppc64** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:PPC64/Installation/Base#Choosing_the_right_profile "Handbook:PPC64/Installation/Base") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:PPC64/Installation/Base#Gentoo_ebuild_repository "Handbook:PPC64/Installation/Base") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:PPC64/Working/Features#Portage_features "Handbook:PPC64/Working/Features") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:PPC64/Installation/Base#Optional:_Adding_a_binary_package_host "Handbook:PPC64/Installation/Base") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
