...
Если в качестве системы инициализации используется systemd, UUID разделов соответствуют Discoverable Partition Specification (как это представлено в разделе [Подготовка дисков](/wiki/Handbook:PPC64/Installation/Disks/ru "Handbook:PPC64/Installation/Disks/ru")) и система использует UEFI, то создание fstab можно пропустить, так как systemd автоматически монтирует все разделы, соответствующие данной спецификации.
...
В **ppc64** примере, /boot является обычным /dev/sda1 разделом с рекомендуемой файловой системой xfs. Необходимо проверять его во время загрузки, поэтому мы запишем следующее:
...
# В данном примере демонстрируются дисковые метки GPT с набором Discoverable Partition Specification (DSP) UUID:
...
Для систем с OpenRC, более подробное руководство по настройке сети доступно в разделе [Расширенная настройка сети](/wiki/Handbook:PPC64/Networking/Introduction/ru "Handbook:PPC64/Networking/Introduction/ru"), который находится ближе к концу руководства. Системы с более специфическими сетевыми потребностями могут пропустить этот раздел, а затем вернуться сюда, чтобы продолжить установку.
...
Чтобы установить имя хоста для системы с _уже запущенным_ systemd, можно воспользоваться утилитой hostnamectl. Однако, во время установки вместо неё следует использовать команду [systemd-firstboot](/wiki/Handbook:PPC64/Installation/System/ru#Init_and_boot_configuration_systemd "Handbook:PPC64/Installation/System/ru") (см. ниже).
...
Больше информации о настройке сети, в том числе об объединении интерфейсов, создании мостов, настройке 802.1Q VLAN и беспроводной сети, рассматриваются в разделе [Настройка сети](/wiki/Handbook:PPC64/Networking/Introduction/ru "Handbook:PPC64/Networking/Introduction/ru").
...
[← Настройка ядра Linux](/wiki/Handbook:PPC64/Installation/Kernel/ru "Предыдущая часть") [К содержанию](/wiki/Handbook:PPC64/ru "Handbook:PPC64/ru") [Установка системных средств →](/wiki/Handbook:PPC64/Installation/Tools/ru "Следующая часть")