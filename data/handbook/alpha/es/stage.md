...
After the stage file download completes, it is possible to verify the integrity and validate the contents of the stage file. Those interested should proceed to the [next section](/wiki/Handbook:Alpha/Installation/Stage/es#Verifying_and_validating "Handbook:Alpha/Installation/Stage/es").

Those not interested in verifying and validating the stage file can close the command-line browser by pressing `q` and can move directly to the [Unpacking the stage file](/wiki/Handbook:Alpha/Installation/Stage/es#Unpacking_the_stage_file "Handbook:Alpha/Installation/Stage/es") section.
...
For all non-official live images which offer gpg and wget in the live environment, refer to the earlier [Verifying the downloaded files](/wiki/Handbook:Alpha/Installation/Media/es#Verifying_the_downloaded_files "Handbook:Alpha/Installation/Media/es") section.
...
Now that the stage file is unpacked, proceed with [Configuring compile options](/wiki/Handbook:Alpha/Installation/Stage/es#Configuring_compile_options "Handbook:Alpha/Installation/Stage/es").
...
CÓDIGO **Example CFLAGS and CXXFLAGS variables**
...
Although the [GCC optimization](/wiki/GCC_optimization/es "GCC optimization/es") article has more information on how the various compilation options can affect a system, the [Safe CFLAGS](/wiki/Safe_CFLAGS "Safe CFLAGS") article may be a more practical place for beginners to start optimizing their systems.
...
ARCHIVO **`/etc/portage/make.conf`** **RUSTFLAGS Example**
...
ARCHIVO **`/etc/portage/make.conf`** **Example MAKEOPTS declaration**
