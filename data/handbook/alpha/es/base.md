...
If the Gentoo installation is interrupted anywhere after this point, it _should_ be possible to 'resume' the installation at this step. There is no need to re-partition the disks again! Simply check the date and time, [mount the root partition](/wiki/Handbook:Alpha/Installation/Disks/es#Mounting_the_root_partition "Handbook:Alpha/Installation/Disks/es") and run the steps above starting with [copying the DNS info](/wiki/Handbook:Alpha/Installation/Base/es#Copy_DNS_info "Handbook:Alpha/Installation/Base/es") to re-enter the working environment. This is also useful for fixing bootloader issues. More information can be found in the [chroot](/wiki/Chroot/es "Chroot/es") article.
...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:Alpha/Installation/Base/es#Gentoo_ebuild_repository "Handbook:Alpha/Installation/Base/es") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **alpha** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:Alpha/Installation/Base/es#Choosing_the_right_profile "Handbook:Alpha/Installation/Base/es") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:Alpha/Installation/Base/es#Gentoo_ebuild_repository "Handbook:Alpha/Installation/Base/es") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.


ARCHIVO **`/etc/portage/binrepos.conf/gentoo.conf`** **CDN-based binary package host example**
...
ARCHIVO **`/etc/portage/make.conf`** **Configure Portage to use binary packages by default**
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:Alpha/Working/Features/es#Portage_features "Handbook:Alpha/Working/Features/es") of the handbook.
...
ARCHIVO **`/etc/portage/make.conf`** **Enabling flags for a KDE/Plasma-based system with DVD, ALSA, and CD recording support**
...
ARCHIVO **`/etc/portage/make.conf`** **Ignoring default USE flags**
...
GPU
VIDEO\_CARDS
(Official) Nvidia Maxwell and newer`nvidia`Nouveau Nvidia [Supported list](/wiki/NVIDIA/es "NVIDIA/es")`nouveau`AMD since Sea Islands`amdgpu radeonsi`ATI and older AMDSee [radeon#Feature support](/wiki/Radeon#Feature_support "Radeon")Intel Nehalem and newer`intel`Intel Gen 2 and 3 [Supported list](/wiki/Intel#.23Feature_support.2Fes "Intel")`intel i915`QEMU/KVM`virgl`WSL`d3d12`

Below is a few examples of a properly set package.use for _VIDEO\_CARDS_:

ARCHIVO **`/etc/portage/package.use/00video_cards`** **AMD example**
...
Details for various GPU(s) can be found at the [AMDGPU](/wiki/AMDGPU "AMDGPU"), [Intel](/wiki/Intel "Intel"), [Nouveau (Open Source)](/wiki/Nouveau/es "Nouveau/es"), or [NVIDIA (Proprietary)](/wiki/NVIDIA/es "NVIDIA/es") articles.
...
ARCHIVO **`/etc/portage/make.conf`** **Accept licenses with ACCEPT\_LICENSE system wide**
...
ARCHIVO **`/etc/portage/package.license/kernel`** **Accepting licenses on a per-package basis**
...
Readers who added a binary host [above](/wiki/Handbook:Alpha/Installation/Base/es#Optional:_Adding_a_binary_package_host "Handbook:Alpha/Installation/Base/es") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
...
ARCHIVO **`/etc/locale.gen`** **Enabling US and DE locales with the appropriate character formats**
...
ARCHIVO **`/etc/env.d/02locale`** **Manually setting system locale definitions**
...
For additional guidance through the locale selection process, read also the [Localization guide](/wiki/Localization/Guide/es "Localization/Guide/es") and the [UTF-8](/wiki/UTF-8/es "UTF-8/es") guide.
...
[← Installing the stage file](/wiki/Handbook:Alpha/Installation/Stage "Previous part") [Home](/wiki/Handbook:Alpha "Handbook:Alpha") [Configuring the kernel →](/wiki/Handbook:Alpha/Installation/Kernel "Next part")