...
ARCHIVO **`/etc/fstab`** **An example DOS/Legacy BIOS boot line for /etc/fstab**
...
ARCHIVO **`/etc/fstab`** **A full /etc/fstab example for a DOS/Legacy BIOS system**
...
For systems running OpenRC, a more detailed reference for network setup is available in the [advanced network configuration](/wiki/Handbook:Alpha/Networking/Introduction/es "Handbook:Alpha/Networking/Introduction/es") section, which is covered near the end of the handbook. Systems with more specific network needs may need to skip ahead, then return here to continue with the rest of the installation.

For more specific systemd network setup, please review see the [networking portion](/wiki/Systemd/es#Network "Systemd/es") of the [systemd](/wiki/Systemd/es "Systemd/es") article.
...
More detailed information about networking, including advanced topics like bonding, bridging, 802.1Q VLANs or wireless networking is covered in the [advanced network configuration](/wiki/Handbook:Alpha/Networking/Introduction/es "Handbook:Alpha/Networking/Introduction/es") section.
...
The next time the system boots, dhcpcd should obtain an IP address from the DHCP server. See the [Dhcpcd](/wiki/Dhcpcd/es "Dhcpcd/es") article for more details.
...
First add the [networkmanager](https://packages.gentoo.org/useflags/networkmanager) [USE flag/es (page does not exist)](/index.php?title=USE_flag/es&action=edit&redlink=1 "USE flag/es (page does not exist)") USE flag to /etc/portage/make.conf:
...
ARCHIVO **`/etc/portage/package.use/networkmanager`**
...
This assumes that the network interface will be called eth0. This is, however, very system dependent. It is recommended to assume that the interface is named the same as the interface name when booted from the installation media if the installation media is sufficiently recent. More information can be found in the [Network interface naming](/wiki/Handbook:Alpha/Networking/Advanced/es#Network_interface_naming "Handbook:Alpha/Networking/Advanced/es") section.

ARCHIVO **`/etc/conf.d/net`** **Static IP definition**
...
ARCHIVO **`/etc/conf.d/net`** **DHCP definition**
...
[systemd](/wiki/Systemd/es "Systemd/es") has it's own network management like OpenRC has netifrc.
...
ARCHIVO **`/etc/systemd/network/50-dhcp.network`** **DHCP**
...
ARCHIVO **`/etc/systemd/network/50-static.network`** **Static**
...
ARCHIVO **`/etc/systemd/network/50-wireless.network`** **WiFi**
...
ARCHIVO **`/etc/hosts`** **Filling in the networking information**
