...
The repository configuration for a binhost is found in Portage's /etc/portage/binrepos.conf/ directory, which functions similarly to the configuration mentioned in the [Gentoo ebuild repository](/wiki/Handbook:Alpha/Installation/Base/pl#Gentoo_ebuild_repository "Handbook:Alpha/Installation/Base/pl") section.

When defining a binary host, there are two important aspects to consider:

1. The architecture and profile targets within the `sync-uri` value _do_ matter and should align to the respective computer architecture ( **alpha** in this case) and system profile selected in the [Choosing the right profile](/wiki/Handbook:Alpha/Installation/Base/pl#Choosing_the_right_profile "Handbook:Alpha/Installation/Base/pl") section.
2. Selecting a fast, geographically close mirror will generally shorten retrieval time. Review the mirrorselect tool mentioned in the [Optional: Selecting mirrors](/wiki/Handbook:Alpha/Installation/Base/pl#Gentoo_ebuild_repository "Handbook:Alpha/Installation/Base/pl") section or review the [online list of mirrors](https://www.gentoo.org/downloads/mirrors/) where URL values can be discovered.
...
Additional Portage features will be discussed in the [the next chapter](/wiki/Handbook:Alpha/Working/Features/pl#Portage_features "Handbook:Alpha/Working/Features/pl") of the handbook.
...
Readers who added a binary host [above](/wiki/Handbook:Alpha/Installation/Base/pl#Optional:_Adding_a_binary_package_host "Handbook:Alpha/Installation/Base/pl") can add --getbinpkg (or -g) in order to fetch packages from the binary host instead of compiling them:
